...
import src.utilities.GoBackToPrevHolder;
...
public class PrefixTreeOptBiSim implements IPrefixTree {
	public double threshold;
	int cntVisualization = 0;
	SnapshotTopkMonitoringOptBiSim snapshotTopkMonitoring;
	...
	private String focusesItSelf;
	private SnapOrStat snapOrStat;
	private double alpha = 1;

	public PrefixTreeOptBiSim(SnapshotTopkMonitoringOptBiSim snapshotTopkMonitoring) throws Exception {
		this.snapshotTopkMonitoring = snapshotTopkMonitoring;
		this.focusSetPath = snapshotTopkMonitoring.focusSetPath;
		this.focusesItSelf = snapshotTopkMonitoring.focusesItSelf;
		...
		this.threshold = snapshotTopkMonitoring.threshold;
		this.alpha = snapshotTopkMonitoring.alpha;
		this.snapOrStat = SnapOrStat.Snap;
	 ...
	 * @throws Exception
	 */
	public PrefixTreeOptBiSim(String focusSetPath, int maxAllowedHops, int maxAllowedEdges,
			GraphDatabaseService dataGraph, boolean debugMode, Bitmap bitMap,
			MinMaxPriorityQueue<PrefixTreeNode<IPrefixTreeNodeData>> topKFrequentPatterns, String dataGraphPath,
			SnapOrStat snapOrStat, double threshold) throws Exception {
	...
	private PrefixTreeNode<IPrefixTreeNodeData> initializePrefixTree() throws Exception {
		...
		if (!DummyProperties.softwareMode) {
			...
			fillSetFromFocuses(focusesItSelf);
		}
		...
		IPrefixTreeNodeData emptyPTRootData = new PrefixTreeNodeDataOptBiSim(focusLabelSet);
		...
		labelAdjacencyIndexer = new Indexer(dataGraph, 1, allNodesOfFocusType,
				snapshotTopkMonitoring != null ? snapshotTopkMonitoring.finalDataGraphPath : dataGraphPath, snapOrStat);
		...
		IPrefixTreeNodeData firstLevelChildData = new PrefixTreeNodeDataOptBiSim(focusNode, dgGraphMatchNodes,
		...
		labelAdjacencyIndexer.generateDistinctNodesAdjMatrix(maxAllowedHops,
				snapshotTopkMonitoring != null ? snapshotTopkMonitoring.focusesItSelf : whatIsFocus);
		if (DummyProperties.debugMode)
			System.out.println(
		...
		while (!traversalQueue.isEmpty()) {
			...
			try {
				...
				if (wasMFP || wasMIP || !tempProcessingNode.getData().isVisited()
					...
					DualSimulationHandler.computeSupport(dataGraph, tempProcessingNode, snapshot, this, alpha);
					...
					if (/* maxAllowedHops > 1 && */ !tempProcessingNode.getData().isDanglingPattern()
						...
						createdDangling = createDanglingPrefixTreeNodesIfNeeded(tempProcessingNode,
								newCreatedOrTouchedPTNodes, 0, alpha);
				...
				// if it's not frequent right now
					...
					tempProcessingNode.getData().maxFreqToNonFreqHandling(tempProcessingNode, this, snapshot, alpha);
				}
			...
			processQueueNode2(dataGraph, traversalQueue, prefixTreeProcessingLevel + 1, tempProcessingNode,
	...
	private boolean createDanglingPrefixTreeNodesIfNeeded(PrefixTreeNode<IPrefixTreeNodeData> tempProcessingNode,
			HashSet<Integer> newCreatedOrTouchedPTNodes, int snapshot, double alpha) throws Exception {
		// if (tempProcessingNode.getData().getPatternPrefixTreeNodeIndex() ==
		// 175) {
		// System.out.println("");
		// }
					...
					if (prevDirection == Direction.OUTGOING) {
						...
						if (max > tempProcessingNode.getData().getFrequencyOfNextNeighborOfSameType()
							...
							isCreated = innerDanglingCreation(oldDanglingPatternNode, remainingNodeIds,
									...
									prevMinPatternNode, newCreatedOrTouchedPTNodes, prevDirection, alpha);
						}
					} else {
						for (Integer nodeId : tempProcessingNode.getData().getMatchedNodes()
								.getDataGraphMatchNodeOfAbsPNode().get(prevMinPatternNode)) {
	...
	private void createFromOldDangling(PatternNode oldDanglingPatternNode, HashSet<Integer> remainingNodeIds,
			PrefixTreeNode<IPrefixTreeNodeData> tempProcessingNode, int snapshot, double alpha) throws Exception {
	...
	private boolean innerDanglingCreation(PatternNode oldDanglingPatternNode, HashSet<Integer> remainingNodeIds,
			...
			Direction prevDirection, double alpha) throws Exception {
		boolean isCreated = false;

		if (oldDanglingPatternNode != null) {
			createFromOldDangling(oldDanglingPatternNode, remainingNodeIds, tempProcessingNode, snapshot, alpha);
		} else {
	...
	public PrefixTreeNode<IPrefixTreeNodeData> generatePrefixTreeForG0() throws Exception {
		...
		while (!traversalQueue.isEmpty()) {
			...
			try {
				...
				// // check for dual-simulation in an incremental way
				DualSimulationHandler.computeSupport(dataGraph, tempProcessingNode, 0, this, alpha);
				...
				if (/* maxAllowedHops > 1 && */ !tempProcessingNode.getData().isDanglingPattern()
						&& tempProcessingNode.getData().getPatternGraph().edgeSet().size() >= 2
						&& tempProcessingNode.getData().getPatternGraph().edgeSet().size() < maxAllowedEdges) {
					createdDangling = createDanglingPrefixTreeNodesIfNeeded(tempProcessingNode, null, 0, alpha);
				}
				// if
				// (tempProcessingNode.getData().getPatternPrefixTreeNodeIndex()
				// == 11) {
				// System.out.println();
				// for(PatternNode patternNode :
				// tempProcessingNode.getData().getPatternGraph().vertexSet()){
				// System.out.println(tempProcessingNode.getData().getMatchedNodes().getDataGraphMatchNodeOfAbsPNode().get(patternNode));
				// }
				// }
			...
			// if (createdDangling) {
			// if (DummyProperties.debugMode)
			// System.out.println("createdDangling: " +
			// tempProcessingNode.getData());
			// continue;
			// }
			...
			processQueueNode2(dataGraph, traversalQueue, prefixTreeProcessingLevel + 1, tempProcessingNode, null,
	...
	private void fillFocusNodesOfRequestedTypes(GraphDatabaseService dataGraph2) throws Exception {
		...
		// sample over focuses if there are large set
		int focusBound = maxAllowedHops == 1 ? 5000 : 2000;
		int degreeBound = 1000;
		for (String key : allNodesOfFocusType.keySet()) {
			if (allNodesOfFocusType.get(key).size() > focusBound) {
				List<Integer> focusList = new ArrayList<Integer>(allNodesOfFocusType.get(key));
				Collections.shuffle(focusList);
				allNodesOfFocusType.get(key).clear();
				for (int i = 0; i < focusBound; i++) {
					if (dataGraph.getNodeById(focusList.get(i)).getDegree() > degreeBound) {
						//System.out.println("deg filterized!");
						focusBound++;
					} else {
						allNodesOfFocusType.get(key).add(focusList.get(i));
					}
				}
			}
		}
			...
			if (this.labelAdjacencyIndexer.prefixTreeNodesOfALevel.get(prefixTreeProcessingLevel) != null) {
				for (PrefixTreeNode<IPrefixTreeNodeData> prefixTreeSibling : this.labelAdjacencyIndexer.prefixTreeNodesOfALevel
					...
					if (DualSimulationHandler.isBiDualSimulated(newPatternGraph, prefixTreeSibling, this)) {
						...
						// .contains(prefixTreeSibling))
						if (DummyProperties.debugMode) {
							// System.out.println("BATCH: simultanous siblings:
							// add node link from "
							// +
							// tempProcessingNode.getData().getPatternPrefixTreeNodeIndex()
							// + " to "
			...
			if (!itWasBisimulated && this.labelAdjacencyIndexer.prefixTreeNodesOfALevel
				...
				for (PrefixTreeNode<IPrefixTreeNodeData> prefixTreeNode : this.labelAdjacencyIndexer.prefixTreeNodesOfALevel
					...
					if (DualSimulationHandler.isBiDualSimulated(newPatternGraph, prefixTreeNode, this)) {
						...
						if (DummyProperties.debugMode) {
							// System.out.println("BATCH: sameLevels: add node
							// link from "
							// +
							// tempProcessingNode.getData().getPatternPrefixTreeNodeIndex()
							// + " to "
	...
	// private void outgoingExpansion(PrefixTreeNode<IPrefixTreeNodeData>
	// tempProcessingNode,
	// IPrefixTreeNodeData tempProcessingNodeData, PatternNode srcPatternNode,
	// HashMap<PatternNode, PrefixTreeNode<IPrefixTreeNodeData>>
	// seenPatternNodeFromPreviousNodesForThisSrcOutgoing,
	// HashMap<String, PrefixTreeNode<IPrefixTreeNodeData>>
	// newlySeenPatternNodeForThisSrcOutgoing,
	// HashSet<Integer> newCreatedOrTouchedPTNodes, Integer srcDataGpNodeId,
	// String otherNodeLabelRelType,
	// int snapshot, int prefixTreeProcessingLevel,
	// HashMap<PatternNode, HashSet<PrefixTreeNode<IPrefixTreeNodeData>>>
	// newChildrenOfTheSrc) throws Exception {
	//
	// HashSet<Integer> sameLabelNeighborNodes =
	// labelAdjacencyIndexer.distinctNodesOfDHopsAway.get(srcDataGpNodeId)
	...
	//
	// int separatorIndex =
	// otherNodeLabelRelType.lastIndexOf(Dummy.DummyProperties.SEPARATOR_LABEL_AND_RELTYPE);
	// String destLabel = otherNodeLabelRelType.substring(0, separatorIndex);
	// String relationshipType = otherNodeLabelRelType.substring(separatorIndex
	// + 1);
	//
	// // FOR-DEBUG START:
	// // if
	...
	// // FOR-DEBUG END:
	//
	// Integer destStepsFromRoot =
	// tempProcessingNodeData.getStepsFromRootOfPatternNodes().get(srcPatternNode)
	// + 1;
	// if (DummyProperties.debugMode) {
	// System.out.println("outgoingExpansion => srcNodeId:" + srcDataGpNodeId +
	// ", newNodeId:" + newNodeId);
	...
	// srcDataGpNodeId, newNodeId, destLabel, relationshipType,
	// destStepsFromRoot);
	//
	// if
	// (tempProcessingNodeData.getFrequencyOfNextNeighborOfSameType().containsKey(srcPatternNode)
	// &&
	...
	// continue;
	// else if (goBackToPrev &&
	// tempProcessingNodeData.getFrequencyOfNextNeighborOfSameType()
	// .get(srcPatternNode).get(otherNodeLabelRelType) >=
	// sameLabelNeighborNodes.size()) {
	// continue;
	// }
	// }
	//
	...
	//
	// destInDegree +=
	// tempProcessingNodeData.getPatternGraph().inDegreeOf(destPatternNode);
	//
	...
	// .equals(srcPatternNode.getLabel()) &&
	...
	// if (DummyProperties.debugMode) {
	// System.out.println("cont: destInDegree:" + destInDegree + " > in degree
	// in data graph:"
	// + labelAdjacencyIndexer.dataGraphNodeInfos.get(newNodeId).inDegree);
	// }
	// continue;
	...
	// if (goBackToPrev) {
	// if
	// (seenPatternNodeFromPreviousNodesForThisSrcOutgoing.containsKey(destPatternNode))
	// {
	// seenPrefixTreeNode =
	// seenPatternNodeFromPreviousNodesForThisSrcOutgoing.get(destPatternNode);
	// }
	// } else {
	...
	// .get(newNodeType + DummyProperties.SEPARATOR_LABEL_AND_RELTYPE +
	// relationshipType);
	// }
	// }
	//
	...
	// seenPrefixTreeNode.getData().getTargetPatternNode();
	//
	...
	// }
	//
	// seenPatternNodeFromPreviousNodesForThisSrcOutgoing.put(tempDestPatternNode,
	// seenPrefixTreeNode);
	//
	// if (newCreatedOrTouchedPTNodes != null)
	...
	// ListenableDirectedGraph<PatternNode, DefaultLabeledEdge> newPatternGraph
	// = new ListenableDirectedGraph<PatternNode, DefaultLabeledEdge>(
	// DefaultLabeledEdge.class);
	//
	// for (PatternNode patternNode :
	// tempProcessingNode.getData().getPatternGraph().vertexSet()) {
	...
	//
	// for (DefaultLabeledEdge e :
	// tempProcessingNode.getData().getPatternGraph().edgeSet()) {
	...
	// }
	//
	// newPatternGraph.addEdge(srcPatternNode, destPatternNode, new
	// DefaultLabeledEdge(relationshipType));
	//
	// // if
	...
	// // }
	//
	// addNewChildrenOrMatches(tempProcessingNode, tempProcessingNodeData,
	// newPatternGraph, srcPatternNode,
	...
	// Dummy.DummyProperties.Direction.OUTGOING, prefixTreeProcessingLevel,
	// newChildrenOfTheSrc);
	//
	// }
	// }
	...
	private void outgoingExpansion2(PrefixTreeNode<IPrefixTreeNodeData> tempProcessingNode,
			...
			String unSeenFocusType) throws Exception {
		...
		for (GoBackToPrevHolder destPatternNode : destPatternNodes) {
			...
			// .get(otherNodeLabelRelType) > 0)
			// continue;
			// else if (destPatternNode.goBackToPrev &&
			// tempProcessingNodeData.getFrequencyOfNextNeighborOfSameType().get(srcPatternNode)
	...
	// Queue<PrefixTreeNode<IPrefixTreeNodeData>> traversalQueue, int
	// prefixTreeProcessingLevel,
	// PrefixTreeNode<IPrefixTreeNodeData> tempProcessingNode, HashSet<Integer>
	// newCreatedOrTouchedPTNodes,
	...
	//
	// HashMap<PatternNode, HashSet<PrefixTreeNode<IPrefixTreeNodeData>>>
	// newChildrenOfTheSrc = new HashMap<PatternNode,
	// HashSet<PrefixTreeNode<IPrefixTreeNodeData>>>();
	// for (PatternNode srcPatternNode :
	// tempProcessingNodeData.getPatternGraph().vertexSet()) {
	// newChildrenOfTheSrc.put(srcPatternNode, new
	// HashSet<PrefixTreeNode<IPrefixTreeNodeData>>());
	//
	...
	// // tempProcessingNodeData.getNewUnexpandedPatternsNodesOfNeo4jNodes()
	// // == null why it's null?!
	// if (tempProcessingNodeData.getNewUnexpandedPatternsNodesOfNeo4jNodes() ==
	// null
	// ||
	...
	// >= maxAllowedHops) {
	// if (DummyProperties.debugMode)
	// System.out.println("maxAllowedHops for srcAbstractPatternNode:" +
	...
	//
	// if (DummyProperties.debugMode) {
	// System.out.println("srcAbstractPatternNode:" + srcPatternNode.getType() +
	// "" + srcPatternNode.isFocus()
	// + " hashCode:" + srcPatternNode.hashCode() + " stepsFromRoot: "
	// +
	// tempProcessingNodeData.getStepsFromRootOfPatternNodes().get(srcPatternNode));
	//
	// if (tempProcessingNodeData.getPatternPrefixTreeNodeIndex() == 3
	// && srcPatternNode.getType().toLowerCase().trim().equals("s")) {
	// System.out.println();
	// }
	// }
	//
	...
	// HashMap<String, PrefixTreeNode<IPrefixTreeNodeData>>
	// newlySeenPatternNodeForThisSrcOutgoing = new HashMap<String,
	// PrefixTreeNode<IPrefixTreeNodeData>>();
	// HashMap<PatternNode, PrefixTreeNode<IPrefixTreeNodeData>>
	// seenPatternNodeFromPreviousNodesForThisSrcOutgoing = new
	// HashMap<PatternNode, PrefixTreeNode<IPrefixTreeNodeData>>();
	//
	// HashMap<String, PrefixTreeNode<IPrefixTreeNodeData>>
	// newlySeenPatternNodeForThisSrcIncoming = new HashMap<String,
	// PrefixTreeNode<IPrefixTreeNodeData>>();
	// HashMap<PatternNode, PrefixTreeNode<IPrefixTreeNodeData>>
	// seenPatternNodeFromPreviousNodesForThisSrcIncoming = new
	// HashMap<PatternNode, PrefixTreeNode<IPrefixTreeNodeData>>();
	//
	// // int matchGraphIndex = -1;
	// // for all match nodes in this prefix-tree node and for this src
	// // pattern node
	// for (Integer srcDataGpNodeId :
	...
	// {
	// if
	// (!tempProcessingNodeData.getNewUnexpandedNodesOfPatternNodes().keySet()
	...
	// // ???
	// if
	// (!tempProcessingNodeData.getNewUnexpandedNodesOfPatternNodes().get(srcDataGpNodeId)
	// .contains(srcPatternNode)) {
	// continue;
	// }
	// }
	...
	// seenPatternNodeFromPreviousNodesForThisSrcOutgoing,
	// newlySeenPatternNodeForThisSrcOutgoing,
	// newCreatedOrTouchedPTNodes, srcDataGpNodeId, snapshot,
	// newChildrenOfTheSrc,
	...
	// for (String otherNodeLabelRelType :
	// labelAdjacencyIndexer.distinctNodesOfDHopsAway
	...
	//
	// outgoingExpansion(tempProcessingNode, tempProcessingNodeData,
	// srcPatternNode,
	// seenPatternNodeFromPreviousNodesForThisSrcOutgoing,
	// newlySeenPatternNodeForThisSrcOutgoing, newCreatedOrTouchedPTNodes,
	// srcDataGpNodeId,
	// otherNodeLabelRelType, snapshot, prefixTreeProcessingLevel,
	// newChildrenOfTheSrc);
	// }
	// for (String otherNodeLabelRelType :
	// labelAdjacencyIndexer.distinctNodesOfDHopsAway
	...
	//
	// incomingExpansion(tempProcessingNode, tempProcessingNodeData,
	// srcPatternNode,
	// seenPatternNodeFromPreviousNodesForThisSrcIncoming,
	...
	// }
	//
	private void processQueueNode2(GraphDatabaseService dataGraph,
		...
		for (PatternNode srcPatternNode : tempProcessingNodeData.getPatternGraph().vertexSet()) {
			...
			if (DummyProperties.debugMode) {
				// System.out.println("srcAbstractPatternNode:" +
				// srcPatternNode.getType() + "" + srcPatternNode.isFocus()
				// + " hashCode:" + srcPatternNode.hashCode() + " stepsFromRoot:
				// "
				// +
				// tempProcessingNodeData.getStepsFromRootOfPatternNodes().get(srcPatternNode));
				...
				if (!tempProcessingNode.getData().getFoundAllFocuses()) {
					...
					for (String otherNodeLabelRelType : labelAdjacencyIndexer.distinctNodesOfDHopsAway
						...
						outgoingExpansion2(tempProcessingNode, tempProcessingNodeData, srcPatternNode,
								...
								otherNodeLabelRelType, snapshot, prefixTreeProcessingLevel, newChildrenOfTheSrc, null);
					}
					for (String otherNodeLabelRelType : labelAdjacencyIndexer.distinctNodesOfDHopsAway
							.get(srcDataGpNodeId).keySet()) {
		...
		for (String otherNodeLabelRelType : labelAdjacencyIndexer.distinctNodesOfDHopsAway.get(srcDataGpNodeId)
			...
			for (String unSeenFocusType : tempProcessingNode.getData().getTypeOfUnSeenFocusNodes()) {
				...
				outgoingExpansion2(tempProcessingNode, tempProcessingNodeData, srcPatternNode,
						...
						prefixTreeProcessingLevel, newChildrenOfTheSrc, unSeenFocusType);
			}
		}
		for (String otherNodeLabelRelType : labelAdjacencyIndexer.distinctNodesOfDHopsAway.get(srcDataGpNodeId)
				.keySet()) {
		...
		for (Integer newNodeId : sameLabelNeighborNodes) {
			...
			for (PatternNode destPatternNode : destPatternNodes) {
				...
				// labelAdjacencyIndexer.dataGraphNodeInfos.get(newNodeId).outDegree
					...
					// if (DummyProperties.debugMode) {
					// System.out.println("cont: destOutDegree:" + destOutDegree
					// + " > out degree in data graph:"
					// +
					// labelAdjacencyIndexer.dataGraphNodeInfos.get(newNodeId).outDegree);
					// }
					continue;
	...
	private void incomingExpansion2(PrefixTreeNode<IPrefixTreeNodeData> tempProcessingNode,
		...
		for (GoBackToPrevHolder destPatternNode : destPatternNodes) {
			...
			// if
			// (tempProcessingNodeData.getFrequencyOfPrevNeighborOfSameType().containsKey(srcPatternNode)
			// &&
			// tempProcessingNodeData.getFrequencyOfPrevNeighborOfSameType().get(srcPatternNode)
			// .containsKey(otherNodeLabelRelType)) {
			// if (!destPatternNode.goBackToPrev &&
	...
	private PrefixTreeNode<IPrefixTreeNodeData> createNewPrefixTreeNode(
		...
		IPrefixTreeNodeData prefixTreeNodeData = new PrefixTreeNodeDataOptBiSim(newPatternGraph,
		...
		if (DummyProperties.debugMode) {
			...
			// System.out.println("newChild:" + newChild.getLevel() + " edgeSet
			// size: "
	...
	private void addIncNodeLink(PrefixTreeNode<IPrefixTreeNodeData> prefixTreeNode,
		...
		if (!prefixTreeNode.getChildrenLinksSet().contains(temp)) {
			if (DummyProperties.debugMode) {
				// System.out.println("INC: add node link from " +
				// prefixTreeNode.getData().getPatternPrefixTreeNodeIndex()
	...
	private void removeDestNode(PrefixTreeNode<IPrefixTreeNodeData> thisNode, Iterator<PatternNode> destIterator,
		...
		if (!thisNode.getData().isValid()) {
			...
			if (wasMFP) {
				thisNode.getData().maxFreqToNonFreqHandling(thisNode, this, snapshot, alpha);
			} else if (wasFrequent) {
	...
	private void updateDeleteAffectedPrefixTreeNode(PrefixTreeNode<IPrefixTreeNodeData> thisNode,
		...
		if (wasMFP) {
			...
			DualSimulationHandler.computeSupport(dataGraph, thisNode, snapshot, this, alpha);
	...
	public void shrinkForNewDeletedEdgesInBatch(
		...
		for (PrefixTreeNode<IPrefixTreeNodeData> thisNode : deleteAffectedPatternsArr) {
			...
			for (SourceRelDestTriple sourceRelDestTriple : sourceRelDestTriples) {
				...
				String sourceLabel = dataGraph.getNodeById(srcNodeId).getLabels().iterator().next().name();
				// this.labelAdjacencyIndexer.dataGraphNodeInfos.get(srcNodeId).nodeLabel;
				Integer destNodeId = sourceRelDestTriple.destNodeId;
				String destLabel = dataGraph.getNodeById(destNodeId).getLabels().iterator().next().name();
				// this.labelAdjacencyIndexer.dataGraphNodeInfos.get(destNodeId).nodeLabel;
