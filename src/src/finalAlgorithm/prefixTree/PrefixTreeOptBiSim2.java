...
public class PrefixTreeOptBiSim2 implements IPrefixTree {
	...
	// HashSet<PrefixTreeNode<IPrefixTreeNodeData>> traversalQueueSet;
	...
	private int focusBound;
	private int degreeBound;
	private String selectedFocus;
	private String dataGraphName;

	public PrefixTreeOptBiSim2(SnapshotTopkMonitoringOptBiSim snapshotTopkMonitoring) throws Exception {
		...
		this.dataGraphName = snapshotTopkMonitoring.dataGraphName;
	 ...
	 * @throws Exception
	 */
	public PrefixTreeOptBiSim2(String focusSetPath, int maxAllowedHops, int maxAllowedEdges,
	...
	private void processQueueNode(GraphDatabaseService dataGraph,
			Queue<PrefixTreeNode<IPrefixTreeNodeData>> traversalQueue, int latticeProcessingLevel,
			...
			if (!tempProcessingNode.getData().getFoundAllFocuses()) {
				...
				for (Integer srcDataGpNodeId : srcNodeIds) {
					for (String otherNodeLabelRelType : labelAdjacencyIndexer.distinctNodesOfDHopsAway
						...
						if (labelAdjacencyIndexer.distinctNodesOfDHopsAway.get(srcDataGpNodeId)
	...
	private void incomingExpansion(PrefixTreeNode<IPrefixTreeNodeData> tempProcessingNode,
			...
			HashMap<PatternNode, HashSet<PrefixTreeNode<IPrefixTreeNodeData>>> newChildrenOfTheSrc,
			boolean unSeenFocusType) throws Exception {
		...
		for (String otherNodeLabelRelType : seenRelTypeIncomingSet.keySet()) {
			...
			if (unSeenFocusType && allNodesOfFocusType
					.get(otherNodeLabelRelType.split(DummyProperties.SEPARATOR_LABEL_AND_RELTYPE)[0]) != null) {
			...
			if (sameLabelNeighborNodes.size() > focusBound) {
				List<Integer> neighborsList = new ArrayList<Integer>(sameLabelNeighborNodes);
				Collections.sort(neighborsList);
				sameLabelNeighborNodes.clear();
				int step = (int) Math.ceil((double) neighborsList.size() / (double) focusBound);

				for (int i = 0; i < neighborsList.size(); i += step) {
					sameLabelNeighborNodes.add(neighborsList.get(i));
				}
			}
			// MATCH REDUCTION END
			...
			for (GoBackToPrevHolder destPatternNode : destPatternNodes) {
				...
				PrefixTreeNode<IPrefixTreeNodeData> seenLatticeNode = null;
				if (goBackToPrev) {
				...
				if (seenLatticeNode != null) {
					...
					IPrefixTreeNodeData latticeNodeData = seenLatticeNode.getData();
					...
					if (newCreatedOrTouchedPTNodes != null)
						newCreatedOrTouchedPTNodes.add(seenLatticeNode.getData().getPatternPrefixTreeNodeIndex());
				} else {
					...
					if (labelAdjacencyIndexer.distinctNodesOfDHopsAway.get(newNodeId)
							...
							&& labelAdjacencyIndexer.distinctNodesOfDHopsAway.get(newNodeId)
									.get(srcPatternNode.getLabel() + DummyProperties.SEPARATOR_LABEL_AND_RELTYPE
											+ relationshipType)
									.containsKey(Indexer.BEFORE)
							&& (incomingFromSameType > labelAdjacencyIndexer.distinctNodesOfDHopsAway
									.get(newNodeId).get(srcPatternNode.getLabel()
											+ DummyProperties.SEPARATOR_LABEL_AND_RELTYPE + relationshipType)
	...
	private void addNewChildrenOrMatches(PrefixTreeNode<IPrefixTreeNodeData> tempProcessingNode,
			...
			HashMap<PatternNode, HashSet<PrefixTreeNode<IPrefixTreeNodeData>>> newChildrenOfTheSrc,
			HashSet<Integer> srcNodeIds) throws Exception {
			...
			for (PrefixTreeNode<IPrefixTreeNodeData> child : tempProcessingNode.getChildren()) {
				...
				if (DualSimulationHandler.preBiSimChecking(child.getData().getPatternGraph(), newPatternGraph, this)
					...
					for (PatternNode destPtn : dualBiSimMap.get(destPatternNode.destPatternNode)) {
						...
						// System.out.println();
						// }
						child.getData().addNewMatchSetForUpdate(tempProcessingNode, tempSrcPatternNode, srcNodeIds,
					...
					// if (!traversalQueueSet.contains(child)) {
					...
					if (DualSimulationHandler.preBiSimChecking(prefixTreeNodeByLevel.getData().getPatternGraph(),
						...
						for (PatternNode destPtn : dualBiSimMap.get(destPatternNode.destPatternNode)) {
							prefixTreeNodeByLevel.getData().addNewMatchSetForUpdate(tempProcessingNode,
									tempSrcPatternNode, srcNodeIds, destPtn, newNodeIds, this.labelAdjacencyIndexer);
						}
		...
		if (!itWasBisimulated) {
			...
			if (numberOfPatternsInPrefixTree > 2567) {
				// System.out.println("pattern bound!");
				return;
			}
	...
	private void getDestPatternNodeAndCheckForGoBackToPrev(ArrayList<GoBackToPrevHolder> destPatternNodes,
			IPrefixTreeNodeData tempProcessingNodeData, PatternNode srcPatternNode, HashSet<Integer> srcNodeIds,
		...
		if (remainingNodeIds.size() > 0 && !Collections.disjoint(remainingNodeIds, srcNodeIds)) {
			// handling self-loop
			HashSet<Integer> goBackToPrevNodeIds = new HashSet<Integer>();
			SetView<Integer> loopNodeIds = Sets.intersection(remainingNodeIds, srcNodeIds);
	...
	private PrefixTreeNode<IPrefixTreeNodeData> initializePrefixTree() throws Exception {
		...
		labelAdjacencyIndexer.generateDistinctNodesAdjMatrix(maxAllowedHops,
				(snapshotTopkMonitoring != null && snapshotTopkMonitoring.focusesItSelf != null)
						? snapshotTopkMonitoring.focusesItSelf : whatIsFocus);
		if (DummyProperties.debugMode)
			System.out.println(
	...
	private void fillFocusNodesOfRequestedTypes(GraphDatabaseService dataGraph2) throws Exception {
		...
		HashMap<String, HashSet<Integer>> usefulFocuses = new HashMap<>();
		for (String focusLabel : focusLabelPropValSet.keySet()) {
			...
			usefulFocuses.put(focusLabel, new HashSet<>());
		}
		...
		File storeDir = new File(snapshotTopkMonitoring.finalDataGraphPath);
		...
		Transaction tx1 = finalDataGraph.beginTx();

		if (allNodesOfFocusType.keySet().size() > 1) {
			for (String startFocusType : allNodesOfFocusType.keySet()) {
				for (Integer startNodeId : allNodesOfFocusType.get(startFocusType)) {
					for (Relationship rel : finalDataGraph.getNodeById(startNodeId)
							.getRelationships(org.neo4j.graphdb.Direction.OUTGOING)) {
						Node endNode = rel.getEndNode();
						String endNodeLbl = endNode.getLabels().iterator().next().name().toString();
						if (allNodesOfFocusType.keySet().contains(endNodeLbl)
								&& allNodesOfFocusType.get(endNodeLbl).contains((int) endNode.getId())) {
							usefulFocuses.get(startFocusType).add(startNodeId);
							usefulFocuses.get(endNodeLbl).add((int) endNode.getId());
						}
					}
				}
			}

			for (String focusLbl : allNodesOfFocusType.keySet()) {
				allNodesOfFocusType.get(focusLbl).clear();
				allNodesOfFocusType.get(focusLbl).addAll(usefulFocuses.get(focusLbl));
			}
		...
		// focusBound = maxAllowedHops == 1 ? 5000 : 2000;
		focusBound = 5000;
		if (maxAllowedHops == 1 && allNodesOfFocusType.size() == 1) {
			focusBound = 2000;
		} else if (maxAllowedHops == 1 && allNodesOfFocusType.size() == 2) {
			focusBound = 1000;
		} else if (maxAllowedHops == 2 && allNodesOfFocusType.size() == 1) {
			focusBound = 1000;
		} else if (maxAllowedHops == 2 && allNodesOfFocusType.size() == 2) {
			focusBound = 500;
		} else if (maxAllowedHops == 2) {
			focusBound = 1000;
		}

		degreeBound = 1000;

		if (Dummy.DummyProperties.softwareMode && dataGraphName.toLowerCase().contains("citation")
				&& maxAllowedHops > 1) {
			focusBound = focusBound / 2;
			degreeBound = degreeBound / 2;
		}

		for (String focusLbl : allNodesOfFocusType.keySet()) {
			if (allNodesOfFocusType.get(focusLbl).size() > focusBound) {
				List<Integer> focusList = new ArrayList<Integer>(allNodesOfFocusType.get(focusLbl));
				// Collections.shuffle(focusList);
				allNodesOfFocusType.get(focusLbl).clear();
				int step = (int) Math.ceil((double) focusList.size() / (double) focusBound);

				for (int i = 0; i < focusList.size(); i += step) {
					if (dataGraph.getNodeById(focusList.get(i)).getDegree() > degreeBound) {
						// System.out.println("deg filterized!");
						focusBound++;
					} else {
						allNodesOfFocusType.get(focusLbl).add(focusList.get(i));
					}
				}
			}
		}
		...
		for (String key : allNodesOfFocusType.keySet()) {
			...
			System.out.println("focus: " + key + " -> " + allNodesOfFocusType.get(key).size());
		}
	...
	private PrefixTreeNode<IPrefixTreeNodeData> createNewPrefixTreeNode(
		...
		if (DummyProperties.debugMode) {
			...
			System.out.println("newChild:" + newChild.getLevel() + " edgeSetsize: "
	...
	private void printTreeDualSim(PrefixTreeNode<IPrefixTreeNodeData> rootNode, GraphDatabaseService graphdb,
			PrefixTreeOptBiSim2 prefixTree) {
		Queue<PrefixTreeNode<IPrefixTreeNodeData>> bfsQueue = new LinkedList<PrefixTreeNode<IPrefixTreeNodeData>>();
		bfsQueue.add(rootNode);
