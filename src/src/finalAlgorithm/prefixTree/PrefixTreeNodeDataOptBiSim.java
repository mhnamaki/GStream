...
public class PrefixTreeNodeDataOptBiSim implements IPrefixTreeNodeData {
	...
	public MatchNodesOptBiSim matchNodes = null;
	...
	// usage: for root node;
	public PrefixTreeNodeDataOptBiSim(Set<String> set) {
	 ...
	 */
	public PrefixTreeNodeDataOptBiSim(PatternNode sourceAbstractPatternNode, HashSet<Integer> srcDataGraphPatternNodes,
		...
		matchNodes = new MatchNodesOptBiSim(dataGraphNodeOfAbsPttnNode, abstractPatternNodeOfNeo4jNode);
	 ...
	 * @param direction
	 */
	public PrefixTreeNodeDataOptBiSim(ListenableDirectedGraph<PatternNode, DefaultLabeledEdge> newAbsPattern,
		...
		if (matchNodes == null) {
			...
			this.matchNodes = new MatchNodesOptBiSim(dataGraphMatchNodeOfAbsPNode, patternNodeOfNeo4jNode);
		}
	...
	private boolean twoPatternNodesAreSame(PatternNode unexpandedPatternNode,
			...
			PrefixTreeNodeDataOptBiSim prefixTreeNodeData) {
	...
	public String getMappedGraphString() {
		...
		if (this.patternGraph != null) {
			...
			returnValue += "\n";
			returnValue += " support? " + this.totalSupportFrequency + "\n";

			// returnValue += "newUnexpandedNodesOfPatternNodes: " +
			// this.newUnexpandedNodesOfPatternNodes + "\n";
			// returnValue += "newUnexpandedPatternsNodesOfNeo4jNodes: " +
			// this.newUnexpandedPatternsNodesOfNeo4jNodes
			// + "\n";
		}
		return returnValue;
	}
	...
	public void setSupportFrequency(int snapshot, double supp, double alpha, boolean fromCarryOver) {
		if (!DummyProperties.windowMode) {
			...
			this.totalSupportFrequency = 0;

			if (snapshot > 0) {
				this.totalSupportFrequency = this.supportFrequency[snapshot - 1];
			}

			this.totalSupportFrequency = alpha * this.totalSupportFrequency + supp;
			this.supportFrequency[snapshot] = this.totalSupportFrequency;
			...
			// this.totalSupportFrequency = alpha * this.totalSupportFrequency +
			// supp;
			// } else {
			...
			if (snapshot > 0) {
				this.totalSupportFrequency = this.supportFrequencyWindowing
						.get(Math.min(snapshot - 1, this.supportFrequencyWindowing.size() - 2));
			}

			this.totalSupportFrequency = alpha * this.totalSupportFrequency + supp;

			if (snapshot < DummyProperties.WINDOW_SIZE)
				this.supportFrequencyWindowing.set(snapshot, this.totalSupportFrequency);
			else
				this.supportFrequencyWindowing.set(DummyProperties.WINDOW_SIZE - 1, this.totalSupportFrequency);

			// if (snapshot < DummyProperties.WINDOW_SIZE) {
			// // if (this.patternPrefixTreeNodeIndex == 0) {
	...
	public void makeParentsMFPIfNoOtherFrequentChild(int snapshot, PrefixTreeNode<IPrefixTreeNodeData> thisNode,
			PrefixTreeNode<IPrefixTreeNodeData> parentPrefixTreeNode,
			List<PrefixTreeNode<IPrefixTreeNodeData>> superNodeLinks, IPrefixTree prefixTree, double alpha)
			throws Exception {
		...
		if (parentPrefixTreeNode != null && parentPrefixTreeNode.getData().isValid()) {
			findFreqOrNewMFPInAncestors(parentPrefixTreeNode, prefixTree, snapshot, seenPTNodes, alpha);
		}
		...
		if (superNodeLinks != null) {
			for (PrefixTreeNode<IPrefixTreeNodeData> superNodeLink : superNodeLinks) {
				...
				if (superNodeLink.getData().isValid())
					findFreqOrNewMFPInAncestors(superNodeLink, prefixTree, snapshot, seenPTNodes, alpha);
			}
		}
	}

	private void findFreqOrNewMFPInAncestors(PrefixTreeNode<IPrefixTreeNodeData> parentPrefixTreeNode,
			IPrefixTree prefixTree, int snapshot, HashSet<Integer> seenPTNodes, double alpha) throws Exception {
		...
		while (!ancestorsQueue.isEmpty()) {
			...
			if (tempParent.getData().isValid()) {
				...
				if (tempParent.getData().isValid()
					...
					checkIfThisCanBeMFPSetIt(tempParent, prefixTree, snapshot, alpha);
				}
			}
	...
	private boolean checkIfThisCanBeMFPSetIt(PrefixTreeNode<IPrefixTreeNodeData> parent, IPrefixTree prefixTree,
		...
		while (!descendantsQueue.isEmpty()) {
			...
			if (tempChild.getData().isValid() && !tempChild.getData().isVerified()) {
				DualSimulationHandler.computeSupport(prefixTree.getDataGraph(), tempChild, snapshot, prefixTree, alpha);
			}
	...
	public void shiftSupportsValues() {
		// this.totalSupportFrequency -=
		// this.supportFrequencyWindowing.getFirst();
	...
	@Override
	public void addNewMatchSetForUpdate(PrefixTreeNode<IPrefixTreeNodeData> parentPrefixTreeNode,
			PatternNode srcAbstractPatternNode, HashSet<Integer> srcDataGraphPatternNodeId, PatternNode destAbstractPatternNode,
			HashSet<Integer> destDataGraphPatternNodeIds, Indexer labelAdjacencyIndexer) {
		...
		addNewMatchSet(srcAbstractPatternNode, srcDataGraphPatternNodeId, labelAdjacencyIndexer);
		addNewMatchSet(destAbstractPatternNode, destDataGraphPatternNodeIds, labelAdjacencyIndexer);
