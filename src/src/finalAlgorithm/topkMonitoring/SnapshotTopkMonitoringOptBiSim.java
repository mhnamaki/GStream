...
public class SnapshotTopkMonitoringOptBiSim {
	...
	public void snapshotTopkMonitor() throws Exception {
		...
		prefixTree = new PrefixTreeOptBiSim2(this);

		// STAT Of the DB START
		// int numberOfAllNodes =
		// Dummy.DummyFunctions.getNumberOfAllNodes(dataGraph);
		// int numberOfAllRelationshipsG0 =
		// Dummy.DummyFunctions.getNumberOfAllRels(dataGraph);
		// HashSet<String> differentLabels =
		// Dummy.DummyFunctions.getDifferentLabels(dataGraph);
		// HashSet<String> differentRelTypes =
		// Dummy.DummyFunctions.getDifferentRelType(dataGraph);
		//
		...
		// STAT Of the DB START
		// double avgG0OutDegreeOfFocusNodes =
		// DummyFunctions.getAvgOutDegreeOfFocusNodes(dataGraph, allFocusNodes,
		...
		if (DummyProperties.softwareMode) {
			// TODO: if the number of focuses or their average degree is too
			// much give an alert to the user
		...
		long numberOfBiSimCheckingRequestG0 = prefixTree.getNumberOfBiSimCheckingRequest();
		long numberOfRealBiSimCheckingG0 = prefixTree.getNumberOfRealBiSimChecking();
		double biSimCheckingTimeG0 = prefixTree.getDurationOfBiSimChecking();
		...
		double danglingCreationDurationG0 = prefixTree.danglingCreationDuration;
		int numberOfDanglingG0 = prefixTree.numberOfDangling;

		prefixTree.resetDurationOfBiSimChecking();
		prefixTree.resetNumberIfBiSimChecking();
		...
		if (DummyProperties.visualize) {
			for (int m = 1; m < maxAllowedEdges + 2; m++) {
				Visualizer.visualizeALevel(prefixTree, m, m);
			}
		}
		...
		int i = 0;
		// boolean snapshotIsChanged = true;
		int oldSnapshotYear = startingIncYear - 1900;
		Date oldSnapshotTime = new Date();
		oldSnapshotTime = idsFormatter.parse("2010-06-15T02:00:00");
		Date globalSnapshotTime = oldSnapshotTime;
		...
		while (snapshot < (numberOfSnapshots - 1)) {
			...
			// because we started from G0/W0 (so new E makes other
			// snapshots/windows)
			// TODO: were some snapshot assignment
			...
			if ((prevSnapshot < snapshot)) {
				if (DummyProperties.debugMode)
					System.out.println("changing the snapshot");

				if ((prevSnapshot % DummyProperties.reportingStep) == 0 && DummyProperties.softwareMode) {
					Dummy.DummyFunctions.saveTrendInfo(dataGraph, snapshot, topKFrequentPatterns, topksOfAllSnapshots);
					Dummy.DummyFunctions.saveTheMiddleResults(dataGraph, snapshot, topKFrequentPatterns,
							topkInSnapshots, k);
				}
				carryOverStart = System.nanoTime();
				SupportHelper.carryOverTheSupport(prefixTree, snapshot, startingWindow, alpha);
				prevSnapshot = snapshot;
				oldSnapshotTime = globalSnapshotTime;
			...
			DirectedGraph<Integer, EdgeStreamCacheItem> cacheStreamGraph = new DirectedPseudograph<Integer, EdgeStreamCacheItem>(
					EdgeStreamCacheItem.class);

			edgesStreamCache.clear();

			int allCachedEdges = 0;
			HashMap<Integer, Integer> newlyTouchedFocusNodesOrSrcDestPatternNodes = new HashMap<Integer, Integer>();
			...
			while (allCachedEdges < edgeStreamCacheCapacity) {

				// if (snapshot >= numberOfSnapshots)
				// break;
...
//				if (allEdgesUpdates % 1000 == 0) {
//					System.out.println("allUp:" + allEdgesUpdates);
//				}
				if ((nextStreamEdge.getTimeStamp().getTime() - oldSnapshotTime.getTime()) > MAX_DURATION) {

					// snapshotIsChanged = true;
					globalSnapshotTime = nextStreamEdge.getTimeStamp();
					snapshot++;
					if (DummyProperties.debugMode) {
				...
				// prefixTree.getLabelAdjacencyIndexer().dataGraphNodeInfos.get(srcNodeId).isFocus;

				boolean destIsAFocus = false;
				if (srcLabel != destLabel || !srcIsAFocus) {
					destIsAFocus = allFocusNodes.contains(destNodeId);
					// prefixTree.getLabelAdjacencyIndexer().dataGraphNodeInfos.get(destNodeId).isFocus;
				}

				boolean destIsPotentiallyAFocus = false;
				if (srcLabel == destLabel && srcIsAFocus) {
					destIsPotentiallyAFocus = allFocusNodes.contains(destNodeId);
				}
				...
				if (DummyProperties.debugMode) {
					System.out.println((allCachedEdges + i) + ": " + " nextStreamEdge: " + srcLabel + "_" + srcNodeId
							+ "-" + nextStreamEdge.getRelationshipType() + " -> " + destLabel + "_" + destNodeId
							+ " => isAdded? " + nextStreamEdge.isAdded() + " t:" + nextStreamEdge.getTimeStamp());

					if (srcNode.hasProperty("uri") && destNode.hasProperty("uri")) {
						System.out.println(
								srcNode.getProperty("uri").toString() + "-" + nextStreamEdge.getRelationshipType()
										+ " -> " + destNode.getProperty("uri").toString());

						if (srcNode.getProperty("uri").toString().contains("192.168.5.122")
								|| destNode.getProperty("uri").toString().contains("192.168.5.122")) {
							System.out.println();
						}
					}
				}
				...
				StreamEdge se = handlingDataStream(nextStreamEdge, prefixTree, srcNode, destNode, srcNodeId,
						destNodeId);
				handlingDataStreamDuration += ((System.nanoTime() - handlingDataStreamStartTime) / 1e6);

				if (se == null) {
					// System.out.println("not useful edge");
					continue;
				}
				...
				EdgeStreamCacheItem eCacheStream1 = new EdgeStreamCacheItem(srcNodeId, destNodeId,
						srcLabel + (srcIsAFocus ? "*" : ""), nextStreamEdge.getRelationshipType(),
						destLabel + (destIsAFocus ? "*" : ""), nextStreamEdge.isAdded());

				edgesStreamCache.add(eCacheStream1);

				EdgeStreamCacheItem eCacheStream2 = null;
				if (destIsPotentiallyAFocus && srcIsAFocus) {
					eCacheStream2 = new EdgeStreamCacheItem(srcNodeId, destNodeId, srcLabel,
							nextStreamEdge.getRelationshipType(), destLabel + "*", nextStreamEdge.isAdded());
					edgesStreamCache.add(eCacheStream2);
				}

				if (nextStreamEdge.isAdded()) {
					if (srcIsAFocus)
						newlyTouchedFocusNodesOrSrcDestPatternNodes.putIfAbsent(srcNodeId, 0);
					if (destIsPotentiallyAFocus || destIsAFocus) {
						newlyTouchedFocusNodesOrSrcDestPatternNodes.putIfAbsent(destNodeId, 0);
					}

					if (!srcIsAFocus && !destIsPotentiallyAFocus) {
						// Source
						MinStepHolder minStepHolder = new MinStepHolder();

						findMinStepSrcPatternNodeForInsert(prefixTree, srcNodeId, patternIndexByNodeIdBitMap[srcNodeId],
								minStepHolder);

						if (minStepHolder.minStep < Integer.MAX_VALUE)
							newlyTouchedFocusNodesOrSrcDestPatternNodes.putIfAbsent(srcNodeId, minStepHolder.minStep);

						// Destination
						minStepHolder = new MinStepHolder();

						findMinStepSrcPatternNodeForInsert(prefixTree, destNodeId,
								patternIndexByNodeIdBitMap[destNodeId], minStepHolder);

						if (minStepHolder.minStep < Integer.MAX_VALUE)
							newlyTouchedFocusNodesOrSrcDestPatternNodes.putIfAbsent(destNodeId, minStepHolder.minStep);
					}

					cacheStreamGraph.addVertex(srcNodeId);
					cacheStreamGraph.addVertex(destNodeId);
					cacheStreamGraph.addEdge(srcNodeId, destNodeId, eCacheStream1);
					addedEdges++;
					if (eCacheStream2 != null) {
						cacheStreamGraph.addEdge(srcNodeId, destNodeId, eCacheStream2);
						addedEdges++;
					}
				...
				// if no incoming edge and src is not a focus node and no
				// outgoing edge and dest is not a focus
				if ((dataGraph.getNodeById(srcNodeId).getDegree(Direction.INCOMING) == 0
						// prefixTree.getLabelAdjacencyIndexer().dataGraphNodeInfos.get(srcNodeId).inDegree
						// == 0
						&& !srcIsAFocus)
						&& (dataGraph.getNodeById(srcNodeId).getDegree(Direction.OUTGOING) == 0
								// prefixTree.getLabelAdjacencyIndexer().dataGraphNodeInfos.get(srcNodeId).outDegree
								// == 0
								&& !destIsAFocus)) {
					if (!nextStreamEdge.isAdded()) {
			...
			if (anyNewInsertedEdge) {
				...
				for (Integer nodeIdInGi : newlyTouchedFocusNodesOrSrcDestPatternNodes.keySet()) {

					nodesToBeTraversedQueue.clear();

					// nodeId and hopAway from focus
					nodesToBeTraversedQueue.add(new CacheGraphStreamQueueItem(nodeIdInGi,
							newlyTouchedFocusNodesOrSrcDestPatternNodes.get(nodeIdInGi)));
					...
					while (!nodesToBeTraversedQueue.isEmpty()) {
						...
						if (queueItem.hopAway < maxAllowedHops) {
							...
							for (EdgeStreamCacheItem e : cacheStreamGraph.incomingEdgesOf(queueItem.nodeId)) {
								if (!e.isImportant) {
									e.isImportant = true;
									// importantCounter++;
									nodesToBeTraversedQueue.add(new CacheGraphStreamQueueItem(
											cacheStreamGraph.getEdgeSource(e), queueItem.hopAway + 1));
								}
				...
				if (DummyProperties.debugMode)
					System.out.println("after identifying important edges1");
				...
				if (DummyProperties.debugMode)
					System.out.println("after identifying important edges2");
				...
				for (SourceRelDestTypeTriple srcRelDestKey : edgeStreamsOfSrcRelTypeDest.keySet()) {
					...
					// System.out.println("srcRelDestKey:" + srcRelDestKey);
									...
									for (EdgeStreamCacheItem sameTypeEdgeStreamCacheItem : edgeStreamsOfSrcRelTypeDest
										...
										if (prefixTreeNodeData.getPatternGraph().edgeSet().size() > 1) {
											...
											if (prefixTreeNodeData.getPatternGraph()
													.outDegreeOf(possibleSrcPatternNode) >
											// dataGraph
											// .getNodeById(sameTypeEdgeStreamCacheItem.sourceNodeId)
											// .getDegree(Direction.OUTGOING)
											prefixTree.labelAdjacencyIndexer.dataGraphNodeInfos
													.get(sameTypeEdgeStreamCacheItem.sourceNodeId).outDegree)
												continue;

											if (prefixTreeNodeData.getPatternGraph()
													.inDegreeOf(possibleSrcPatternNode) >
											// dataGraph
											// .getNodeById(sameTypeEdgeStreamCacheItem.sourceNodeId)
											// .getDegree(Direction.INCOMING)
											prefixTree.getLabelAdjacencyIndexer().dataGraphNodeInfos
													.get(sameTypeEdgeStreamCacheItem.sourceNodeId).inDegree)
												continue;

											if (prefixTreeNodeData.getPatternGraph()
													.outDegreeOf(possibleDestPatternNode) >
											// dataGraph
											// .getNodeById(sameTypeEdgeStreamCacheItem.destNodeId)
											// .getDegree(Direction.OUTGOING)
											prefixTree.getLabelAdjacencyIndexer().dataGraphNodeInfos
													.get(sameTypeEdgeStreamCacheItem.destNodeId).outDegree)
												continue;

											if (prefixTreeNodeData.getPatternGraph()
													.inDegreeOf(possibleDestPatternNode) >
											// dataGraph
											// .getNodeById(sameTypeEdgeStreamCacheItem.destNodeId)
											// .getDegree(Direction.INCOMING)
											prefixTree.getLabelAdjacencyIndexer().dataGraphNodeInfos
													.get(sameTypeEdgeStreamCacheItem.destNodeId).inDegree)
												continue;
											...
											if (prefixTreeNodeData.getFrequencyOfNextNeighborOfSameType()
													.containsKey(possibleDestPatternNode)) {
												for (String nextTypeKey : prefixTreeNodeData
														.getFrequencyOfNextNeighborOfSameType()
														.get(possibleDestPatternNode).keySet()) {
													if (!prefixTree.getLabelAdjacencyIndexer().distinctNodesOfDHopsAway
											...
											boolean satisfyPrevType = true;
											if (prefixTreeNodeData.getFrequencyOfPrevNeighborOfSameType()
													.containsKey(possibleSrcPatternNode)) {
												for (String prevTypeKey : prefixTreeNodeData
														.getFrequencyOfPrevNeighborOfSameType()
														.get(possibleSrcPatternNode).keySet()) {

													if (!prefixTree.getLabelAdjacencyIndexer().distinctNodesOfDHopsAway
															.get(sameTypeEdgeStreamCacheItem.sourceNodeId)
															.containsKey(prevTypeKey)
															|| !prefixTree
																	.getLabelAdjacencyIndexer().distinctNodesOfDHopsAway
																			.get(sameTypeEdgeStreamCacheItem.sourceNodeId)
																			.get(prevTypeKey)
																			.containsKey(Indexer.BEFORE)) {
														satisfyPrevType = false;
														break;
													}

													if (prefixTreeNodeData.getFrequencyOfPrevNeighborOfSameType()
															.get(possibleSrcPatternNode)
															.get(prevTypeKey) > prefixTree
																	.getLabelAdjacencyIndexer().distinctNodesOfDHopsAway
																			.get(sameTypeEdgeStreamCacheItem.sourceNodeId)
																			.get(prevTypeKey).get(Indexer.BEFORE)
																			.size()) {
														satisfyPrevType = false;
														break;
													}
											...
											for (String prevTypeKey : prefixTreeNodeData
													.getFrequencyOfPrevNeighborOfSameType().get(possibleDestPatternNode)
													.keySet()) {
												if (!prefixTree.getLabelAdjacencyIndexer().distinctNodesOfDHopsAway
														.get(sameTypeEdgeStreamCacheItem.destNodeId)
														.containsKey(prevTypeKey)
														|| !prefixTree
																.getLabelAdjacencyIndexer().distinctNodesOfDHopsAway
																		.get(sameTypeEdgeStreamCacheItem.destNodeId)
																		.get(prevTypeKey).containsKey(Indexer.BEFORE)) {
													satisfyPrevType = false;
													break;
												}
												...
												if (prefixTreeNodeData.getFrequencyOfPrevNeighborOfSameType()
														...
														.get(prevTypeKey) > prefixTree
																.getLabelAdjacencyIndexer().distinctNodesOfDHopsAway
																		.get(sameTypeEdgeStreamCacheItem.destNodeId)
																		.get(prevTypeKey).get(Indexer.BEFORE).size()) {
													satisfyPrevType = false;
													break;
												}
											}
										...
										bitMap.updateBitmapByPairOfPatternAndNodeId(patternGraphIndex,
												sameTypeEdgeStreamCacheItem.sourceNodeId);
				...
				if (DummyProperties.debugMode)
					System.out.println("after putting matches");
			...
			if (DummyProperties.debugMode)
				System.out.println("after finding affected patterns");
			...
			if (anyNewDeletedEdge) {
				...
				if (DummyProperties.debugMode)
					System.out.println("after deletion handling");
			}
			...
			if (DummyProperties.debugMode) {
				DebugHelper.printAffectedPatterns(insertAffectedPatternsArr);
			}
			// Integer targetPattern = 15;
			// if(insertAffectedPatternsArr.contains(prefixTree.getPrefixTreeNodeIndex().get(targetPattern))){
			// System.out.println("AFFECTED: snapshot: " + snapshot + ",
			// pattern: " +
			// prefixTree.getPrefixTreeNodeIndex().get(targetPattern).getData());
			// }
			// else{
			// System.out.println("not affected snapshot: " + snapshot + ",
			// pattern: " +
			// prefixTree.getPrefixTreeNodeIndex().get(targetPattern).getData());
			// }
			...
			for (int p = 0; p < insertAffectedPatternsArr.size(); p++) {
				...
				!newCreatedOrTouchedPTNodes
					...
					prefixTree.expandForNewInsertedEdge(insertAffectedPatternsArr.get(p), newCreatedOrTouchedPTNodes,
							snapshot, threshold, topKFrequentPatterns);
					// }
				} else {
				...
				if (DummyProperties.debugMode)
					System.out.println("after insertion handling");
			}
			...
			if (DummyProperties.softwareMode) {
				processingTimeOfEdges.put(allEdgesUpdates, (allEdgesResponseTimeDuration / (double) 1000));
			}

			i++;
		}

		// recently added for software START
		if ((prevSnapshot % DummyProperties.reportingStep) == 0 && DummyProperties.softwareMode) {
			Dummy.DummyFunctions.saveTrendInfo(dataGraph, snapshot, topKFrequentPatterns, topksOfAllSnapshots);
			Dummy.DummyFunctions.saveTheMiddleResults(dataGraph, snapshot, topKFrequentPatterns, topkInSnapshots, k);
		}

		SupportHelper.carryOverTheSupport(prefixTree, snapshot, startingWindow, alpha);
		// recently added for software END
		...
		if (DummyProperties.bigDataTestMode) {
			System.out.println("numberOfAllPatterns: " + numberOfAllPatterns);
		}
		if (DummyProperties.visualize) {
			for (int m = 1; m < maxAllowedEdges + 2; m++) {
				Visualizer.visualizeALevel(prefixTree, m, m);
		...
		DummyFunctions.saveTrendInfo(dataGraph, numberOfSnapshots - 1, topKFrequentPatterns, topksOfAllSnapshots);
		Dummy.DummyFunctions.printTrendInfo(topksOfAllSnapshots, prefixTree, numberOfSnapshots, startingWindow);

		if (!DummyProperties.bigDataTestMode) {
			TopKHandler.printTopK(topKFrequentPatterns);
		}

		// double avgGTOutDegreeOfFocusNodes =
		// DummyFunctions.getAvgOutDegreeOfFocusNodes(dataGraph, allFocusNodes,
		// numberOfAllFocusNodes);
		// int numberOfAllRelationshipsGT =
		// Dummy.DummyFunctions.getNumberOfAllRels(dataGraph);
		...
		// timeInfos.add(new InfoHolder(2, "G0 Relationships",
		// numberOfAllRelationshipsG0));
		// timeInfos.add(new InfoHolder(3, "GT Relationships",
		// numberOfAllRelationshipsGT));
		//
		// timeInfos.add(new InfoHolder(4, "Distinct Labels",
		// differentLabels.size()));
		// timeInfos.add(new InfoHolder(5, "Distinct RelTypes",
		// differentRelTypes.size()));
		//
		// timeInfos.add(new InfoHolder(6, "Average of Total Degrees",
		// avgDegrees));
		// timeInfos.add(new InfoHolder(7, "Average of G0 Focus Out Degrees",
		// avgG0OutDegreeOfFocusNodes));
		// timeInfos.add(new InfoHolder(8, "Average of GT Focus Out Degrees",
		// avgGTOutDegreeOfFocusNodes));

		timeInfos.add(new InfoHolder(9, "k", k));
		timeInfos.add(new InfoHolder(10, "Threshold", threshold));
		timeInfos.add(new InfoHolder(11, "Requested Transactions In A Snapshot", numberOfTransactionInASnapshot));
		timeInfos.add(new InfoHolder(12, "Requested Snapshots", numberOfSnapshots));

		timeInfos.add(new InfoHolder(13, "Diameter", maxAllowedHops));
		timeInfos.add(new InfoHolder(14, "Max Edges", maxAllowedEdges));
		timeInfos.add(new InfoHolder(15, "Focus Nodes", numberOfAllFocusNodes));

		timeInfos.add(new InfoHolder(16, "All Patterns", numberOfAllPatterns));
		timeInfos.add(new InfoHolder(17, "All G0 Patterns", numberOfAllG0Patterns));
		timeInfos.add(new InfoHolder(18, "All Inc Patterns", (numberOfAllPatterns - numberOfAllG0Patterns)));

		timeInfos.add(new InfoHolder(19, "TotalAllMatches", prefixTree.numberOfTotalAllMatches));
		timeInfos.add(new InfoHolder(20, "TotalAllMatches G0", initNumberOfTotalAllMatches));
		timeInfos.add(new InfoHolder(21, "TotalAllMatches Just Inc",
				(prefixTree.numberOfTotalAllMatches - initNumberOfTotalAllMatches)));

		timeInfos.add(new InfoHolder(22, "PrefixTree G0 Generation Time", g0PrefixTreeGenerationDuration));

		timeInfos.add(new InfoHolder(23, "All edges response time", allEdgesResponseTimeDuration));
		timeInfos.add(new InfoHolder(24, "All Insertion Response Time", insertionResponseDuration));
		timeInfos.add(
				new InfoHolder(25, "All Insertion For Expansion Response Time", insertionForExpansionResponseDuration));

		timeInfos.add(new InfoHolder(26, "Exp > checkSameTypeSameStepsFromRootHasEnoughMatches",
				prefixTree.checkSameTypeSameStepsFromRootHasEnoughMatchesDuration));
		timeInfos.add(new InfoHolder(27, "Exp > checkValidityAtLeastOneMatchForEachPatternNode",
				prefixTree.checkValidityAtLeastOneMatchForEachPatternNodeDuration));
		timeInfos.add(new InfoHolder(28, "Exp > processQueueNode", prefixTree.processQueueNodeDuration));

		timeInfos.add(
				new InfoHolder(29, "All Insertion For Put Matches Response Time", insertionPutMatchesResponseDuration));
		timeInfos.add(new InfoHolder(30, "All Deletion Response Time", deletionResponseDuration));

		timeInfos.add(new InfoHolder(31, "Total deltaE", (addedEdges + removedEdges)));
		timeInfos.add(new InfoHolder(32, "All inserted edges", addedEdges));
		timeInfos.add(new InfoHolder(33, "All deleted edges", removedEdges));

		timeInfos.add(new InfoHolder(34, "affected inserted edges", (addedEdges - nonAffectedInsertedEdges)));
		timeInfos.add(new InfoHolder(35, "affected deleted edges", (removedEdges - nonAffectedDeletedEdges)));

		timeInfos.add(new InfoHolder(36, "All Affected Patterns In All Rounds", numberOfAffectedPatterns));
		timeInfos.add(new InfoHolder(37, "Affected Patterns Without Recheck", numberOfAffectedPatternsWithoutRecheck));

		timeInfos.add(new InfoHolder(38, "Total physical edge insertion/deletion time", handlingDataStreamDuration));

		timeInfos.add(new InfoHolder(39, "BiSim Checking Time G0", biSimCheckingTimeG0));
		timeInfos.add(new InfoHolder(40, "Number Of BiSim Checking Request G0", numberOfBiSimCheckingRequestG0));
		timeInfos.add(new InfoHolder(41, "Number Of Real BiSim Checking G0", numberOfRealBiSimCheckingG0));
		timeInfos.add(new InfoHolder(42, "Creation Of New PrefixTree Node G0", newPrefixTreeGenerationG0));
		timeInfos.add(new InfoHolder(43, "Num. of support computations G0", numberOfSupportComputationsG0));
		timeInfos.add(new InfoHolder(44, "Support computational time G0", computaionalSupportTimeG0));
		timeInfos.add(new InfoHolder(45, "Creation/Checking Of Dangling Nodes G0", danglingCreationDurationG0));
		timeInfos.add(new InfoHolder(46, "Number of Dangling Nodes G0", numberOfDanglingG0));

		timeInfos.add(new InfoHolder(47, "BiSim Checking Time Inc", prefixTree.getDurationOfBiSimChecking()));
		timeInfos.add(new InfoHolder(48, "Number Of BiSim Checking Request Inc",
				prefixTree.getNumberOfBiSimCheckingRequest()));
		timeInfos.add(
				new InfoHolder(49, "Number Of Real BiSim Checking Inc", prefixTree.getNumberOfRealBiSimChecking()));
		timeInfos.add(new InfoHolder(50, "Creation Of New PrefixTree Node Inc",
				prefixTree.getDurationOfNewPrefixTreeGeneration()));
		timeInfos.add(new InfoHolder(51, "Num. of support computations Inc", prefixTree.getNumberOfComputeSupport()));
		timeInfos.add(new InfoHolder(52, "Support computational time Inc", prefixTree.getDurationOfComputeSupport()));
		timeInfos.add(new InfoHolder(53, "Creation/Checking Of Dangling Nodes Inc",
				(prefixTree.danglingCreationDuration - danglingCreationDurationG0)));
		timeInfos.add(
				new InfoHolder(54, "Number of Dangling Nodes Inc", (prefixTree.numberOfDangling - numberOfDanglingG0)));

		timeInfos.add(new InfoHolder(55, "Induced G duration", creationOfInducedGraphDuration));
		timeInfos.add(new InfoHolder(56, "Update neighborhood info", updateNeighborhoodDuration));
		timeInfos.add(new InfoHolder(57, "Update BitMap Duration", updateBitMapTimeDuration));
		timeInfos.add(new InfoHolder(58, "WindowSize", DummyProperties.WINDOW_SIZE));
		timeInfos.add(new InfoHolder(59, "WindowMode", DummyProperties.windowMode));
		timeInfos.add(new InfoHolder(60, "carryOverDuraion", carryOverDuraion));
		timeInfos.add(new InfoHolder(61, "winMineDuraion", winMineDuraion));

		TimeLogger.LogTime("biSimDis_" + (DummyProperties.windowMode ? "Win" : "Inc") + "_"
				+ DummyProperties.WINDOW_SIZE + "_cache_" + edgeStreamCacheCapacity + ".txt", true, timeInfos);

		// for (Integer patternId : prefixTree.prefixTreeNodeIndex.keySet()) {
		// System.out.println(patternId);
		// for (int d = 0; d <
		// prefixTree.prefixTreeNodeIndex.get(patternId).getData()
		// .getSupportFrequencies().length; d++) {
		// System.out.print(
		// "s" + d + " =" +
		// prefixTree.prefixTreeNodeIndex.get(patternId).getData().getSupportFrequencies()[d]
		// + " - ");
		// }
		//
		// System.out.println();
		...
		if (DummyProperties.softwareMode) {
			Dummy.DummyFunctions.printTopKInfo(topkInSnapshots);
			DummyFunctions.printPerformanceInfo(numberOfAllPatterns, (numberOfAllPatterns - numberOfAllG0Patterns),
					prefixTree.getNumberOfComputeSupport(), addedEdges, removedEdges, numberOfSupportComputationsG0,
					allEdgesUpdates, processingTimeOfEdges);
			DummyFunctions.printFinalJson();
		}

		dataGraph.shutdown();

		bitMap = null;
		prefixTree.getLabelAdjacencyIndexer().relTypeOfSrcAndDest.clear();
		prefixTree.getLabelAdjacencyIndexer().distinctNodesOfDHopsAway.clear();
		prefixTree = null;
	...
	public SnapshotTopkMonitoringOptBiSim(String[] args) throws Exception {
		...
		for (int i = 0; i < args.length; i++) {
			...
			} else if (args[i].equals("-finalDataGraphPath")) {
				finalDataGraphPath = args[++i];
			...
			} else if (args[i].equals("-softwareMode")) {
				DummyProperties.softwareMode = Boolean.parseBoolean(args[++i]);
	...
	public SnapshotTopkMonitoringOptBiSim(Configuration conf) throws Exception {

		// DummyProperties.debugMode = true;
		// debugMode = true;
		DummyProperties.softwareMode = true;
		topkInSnapshots = new JSONArray();
		topksOfAllSnapshots = new HashMap<Integer, HashSet<Integer>>();
		processingTimeOfEdges = new HashMap<Integer, Double>();

		dataGraphName = conf.dataGraphName;
		maxAllowedHops = conf.maxAllowedHops;
		maxAllowedEdges = conf.maxAllowedEdges;
		dataGraphPath = conf.dataGraphPath;
		dateFormat = conf.dateFormat;
		deltaEFileOrFiles = conf.deltaEFileOrFiles;
		k = conf.k;
		alpha = conf.alpha;
		threshold = conf.threshold;
		numberOfTransactionInASnapshot = conf.numberOfTransactionInASnapshot;
		numberOfSnapshots = conf.numberOfSnapshots;
		numberOfIgnoranceInitEdges = 0;
		edgeStreamCacheCapacity = conf.edgeStreamCacheCapacity;

		windowMode = conf.windowMode;
		DummyProperties.windowMode = windowMode;

		windowSizeL = conf.windowSizeL;
		DummyProperties.WINDOW_SIZE = windowSizeL;
		endingWindow = windowSizeL - 1;
		this.finalDataGraphPath = conf.finalDataGraphPath;

		if (conf.startingIncYear > 0)
			this.startingIncYear = conf.startingIncYear;

		this.focusesItSelf = conf.focuses;
		if (conf.dataGraphName.toLowerCase().contains("panama") && conf.focuses.split(",") != null
				&& conf.focuses.split(",").length > 1) {
			maxAllowedHops = 1;
		}
		// focusesItSelf = "Paper|Title:recommendation,Venue_DM";
		// focusesItSelf = "Sudan";

		// if (windowMode)
		// DummyProperties.reportingStep = DummyProperties.WINDOW_SIZE;

		String errors = "";
		if (focusesItSelf == null || focusesItSelf.equals("0")) {
			errors += " focuses ";
		}
		if (dataGraphPath == null) {
			errors += " data graph ";
		}
		if (maxAllowedHops == 0) {
			errors += " radius ";
		...
		if (finalDataGraphPath == null) {
			errors += " final data graph ";
		}

		if (!errors.equals("")) {
			Configuration.returnErrorMessageToWeb("please select the: " + errors);
			return;
		}

		if (maxAllowedEdges < maxAllowedHops) {
			Configuration.returnErrorMessageToWeb(
					" maxAllowedEdges < maxAllowedHops : " + maxAllowedEdges + " < " + maxAllowedHops);
			return;
		}
		...
		if (newDGPath == null) {
			Configuration.returnErrorMessageToWeb(" new data graph path does not find");
			return;
		}
	...
	public SnapshotTopkMonitoringOptBiSim(String focusSetPath, int maxHops, int maxEdges, String dataGraphPath,
	...
	public static void main(String[] args) throws Exception {
		SnapshotTopkMonitoringOptBiSim snapshotTopkMonitoring = new SnapshotTopkMonitoringOptBiSim(args);
				...
				if (prefixTNodeData.getPatternGraph().edgeSet().size() < maxAllowedEdges) {
					for (PatternNode patternNode : prefixTNodeData.getMatchedNodes().getPatternNodeOfNeo4jNode()
						...
						if (prefixTNodeData.getStepsFromRootOfPatternNodes().get(patternNode) < maxAllowedHops) {
							...
							prefixTNodeData.getNewUnexpandedPatternsNodesOfNeo4jNodes().putIfAbsent(patternNode,
									new HashSet<Integer>());
							prefixTNodeData.getNewUnexpandedPatternsNodesOfNeo4jNodes().get(patternNode)
									.add(destNodeId);
							...
							prefixTNodeData.getNewUnexpandedNodesOfPatternNodes().putIfAbsent(destNodeId,
									new HashSet<PatternNode>());
							prefixTNodeData.getNewUnexpandedNodesOfPatternNodes().get(destNodeId).add(patternNode);
	...
	private void findMinStepSrcPatternNodeForInsert(IPrefixTree prefixTree, int srcNodeId,
			RoaringBitmap srcRoaringBitmap, MinStepHolder minStepHolder) {
		...
		srcRoaringBitmap.forEach(new IntConsumer() {
			...
			public void accept(int value) {
				...
				minStepHolder.minStep = Math.min(minSteps, minStepHolder.minStep);
			}
	...
	private StreamEdge handlingDataStream(StreamEdge nextStreamEdge, IPrefixTree prefixTree, Node srcNode,
		...
		// prefixTree.getLabelAdjacencyIndexer().checkForExistenceInNeighborhoodIndex(prefixTree,
		// srcNodeId, destNodeId,
		// srcNode, destNode);

		if (!Indexer.usefulStaticNodes.contains(srcNodeId) || !Indexer.usefulStaticNodes.contains(destNodeId)) {
			return null;
		}

		String relationship = nextStreamEdge.getRelationshipType();
		relationship = relationship.intern();
		...
		if (nextStreamEdge.isAdded()) {
			...
			// if
			// (!prefixTree.getLabelAdjacencyIndexer().dataGraphNodeInfos.get(srcNodeId).nextNodeIds
			// .contains(destNodeId)) {
			...
			if (
			// dataGraph.getNodeById(srcNodeId).getDegree(Direction.OUTGOING) <=
			...
			if (existed) {
				...
				// TODO: patch
				if (prefixTree.getLabelAdjacencyIndexer().relTypeOfSrcAndDest.containsKey(srcNodeId))
