...
import src.bitMapAlg.prefixTree.PrefixTreeBitMap;
...
public class SnapshotTopkMonitoringBitMap {
	...
	public PrefixTreeBitMap prefixTree;
	...
	public void snapshotTopkMonitor() throws Exception {
		...
		prefixTree = new PrefixTreeBitMap(this);
		...
		for (int i = 0; i < numberOfTransactionInASnapshot * (numberOfSnapshots - 1); i++) {
			...
			if (prevSnapshot != snapshot) {
				...
				Dummy.DummyFunctions.saveTrendInfo(dataGraph, snapshot, topKFrequentPatterns);
			}
			...
			while (allCachedEdges < edgeStreamCacheCapacity
					&& ((i + allCachedEdges) < numberOfTransactionInASnapshot * (numberOfSnapshots - 1))) {
			...
			Dummy.DummyFunctions.reportTheMiddleResults(dataGraph, snapshot, topKFrequentPatterns);
			Dummy.DummyProperties.processingTimeOfEdges.put(allEdgesUpdates, allEdgesResponseTimeDuration);
		}

		System.out.println("final traversal");
		int numberOfAllPatterns = prefixTree.bfsTraverse(prefixTree.emptyPTRootNode);

		Dummy.DummyFunctions.printTrendInfo();
		...
		timeInfos.add(new InfoHolder(2, "G0 Relationships", numberOfAllRelationshipsG0));
		timeInfos.add(new InfoHolder(3, "GT Relationships", numberOfAllRelationshipsGT));

		timeInfos.add(new InfoHolder(4, "Distinct Labels", differentLabels.size()));
		timeInfos.add(new InfoHolder(5, "Distinct RelTypes", differentRelTypes.size()));

		timeInfos.add(new InfoHolder(6, "Average of Total Degrees", avgDegrees));
		timeInfos.add(new InfoHolder(7, "Average of G0 Focus Out Degrees", avgG0OutDegreeOfFocusNodes));
		timeInfos.add(new InfoHolder(8, "Average of GT Focus Out Degrees", avgGTOutDegreeOfFocusNodes));
		...
		DummyFunctions.printPerformanceInfo(numberOfAllPatterns, prefixTree.getNumberOfComputeSupport(), addedEdges,
				removedEdges, numberOfSupportComputationsG0);

		dataGraph.shutdown();
	}

	public SnapshotTopkMonitoringBitMap(String[] args) throws Exception {
	...
	public SnapshotTopkMonitoringBitMap(String focusSetPath, int maxHops, int maxEdges, String dataGraphPath,
	...
	public static void main(String[] args) throws Exception {
		SnapshotTopkMonitoringBitMap snapshotTopkMonitoring = new SnapshotTopkMonitoringBitMap(args);
