package src.alg1BiSim.topkMonitoring;
...
import src.alg1BiSim.prefixTree.PrefixTreeAlg1BiSim;
...
public class SnapshotTopkMonitoringBiSim {
	...
	public void snapshotTopkMonitor() throws Exception {
		...
		PrefixTreeAlg1BiSim prefixTree = new PrefixTreeAlg1BiSim(this);
		...
		double biSimCheckingTimeG0 = prefixTree.getDurationOfIsoChecking();
		...
		ArrayList<PrefixTreeNode<IPrefixTreeNodeData>> affectedPatternsArr = new ArrayList<PrefixTreeNode<IPrefixTreeNodeData>>();
		HashSet<PrefixTreeNode<IPrefixTreeNodeData>> affectedPatternsSet = new HashSet<PrefixTreeNode<IPrefixTreeNodeData>>();
		...
		for (int i = 0; i < numberOfTransactionInASnapshot * (numberOfSnapshots - 1); i++) {
			...
			// String relationshipType = nextStreamEdge.getRelationshipType();
			...
			// prefixTree.labelAdjacencyIndexer.dataGraphNodeInfos.get(srcNodeId).isFocus;

			boolean destIsAFocus = false;
			if (srcLabel != destLabel || !srcIsAFocus) {
				destIsAFocus = allFocusNodes.contains(destNodeId);
				// prefixTree.labelAdjacencyIndexer.dataGraphNodeInfos.get(destNodeId).isFocus;
			}

			// if no incoming edge and this is not a focus node
			if (prefixTree.labelAdjacencyIndexer.dataGraphNodeInfos.get(srcNodeId).inDegree == 0 && !srcIsAFocus) {
				if (nextStreamEdge.isAdded()) {
			...
			affectedPatternsSet.clear();
			affectedPatternsArr.clear();
			BooleanInside booleanInsideForIns = new BooleanInside();
			// BooleanInside booleanInsideForDel = new BooleanInside();
			if (nextStreamEdge.isAdded()) {
				findAffectedParentPatternsForInsertAndAddNewUnexpandedMatch(booleanInsideForIns, affectedPatternsSet,
						prefixTree, srcNodeId, destNodeId, patternIndexByNodeIdBitMap[srcNodeId],
						patternIndexByNodeIdBitMap[destNodeId], destLabel);
				affectedPatternsArr.addAll(affectedPatternsSet);
			} else {
				findAffectedPatterns(affectedPatternsArr, prefixTree, srcNodeId, destNodeId,
			...
			if (!srcIsAFocus && affectedPatternsArr.size() == 0) {
				if (nextStreamEdge.isAdded()) {
					nonAffectedInsertedEdges++;
					allInsertionResponseTimes += ((System.nanoTime() - starttime) / 1e6);
				} else {
			...
			numberOfAffectedPatterns += affectedPatternsArr.size();

			sortPatternsInAscending(affectedPatternsArr);

			HashSet<Integer> newCreatedOrTouchedPTNodes = new HashSet<Integer>();

			if (nextStreamEdge.isAdded()) {

				for (int p = 0; p < affectedPatternsArr.size(); p++) {
					...
					!newCreatedOrTouchedPTNodes
							.contains(affectedPatternsArr.get(p).getData().getPatternPrefixTreeNodeIndex())) {
						...
						prefixTree.expandForNewInsertedEdge(affectedPatternsArr.get(p), newCreatedOrTouchedPTNodes,
				...
				for (int p = 0; p < affectedPatternsArr.size(); p++) {
					...
					if (!newCreatedOrTouchedPTNodes
						...
						prefixTree.shrinkForNewDeletedEdge(srcNode, destNode, srcIsAFocus, destIsAFocus,
								affectedPatternsArr.get(p), newCreatedOrTouchedPTNodes, snapshot, threshold,
		...
		timeInfos.add(new InfoHolder(25, "Total Insertion AFF Response Time",
				DummyFunctions.getTotalSumOfDoubleArray(insertionResponseTimes)));

		timeInfos.add(new InfoHolder(26, "Total Deletion AFF Response Time",
				DummyFunctions.getTotalSumOfDoubleArray(deletionResponseTimes)));

		timeInfos.add(new InfoHolder(27, "Avg Insertion AFF Response Time",
				DummyFunctions.getAverageOfDoubleArray(insertionResponseTimes)));

		timeInfos.add(new InfoHolder(28, "Avg Deletion AFF Response Time",
				DummyFunctions.getAverageOfDoubleArray(deletionResponseTimes)));
		...
		timeInfos.add(new InfoHolder(38, "BiSim Checking Time G0", biSimCheckingTimeG0));
		timeInfos.add(new InfoHolder(39, "Number Of BiSim Checking Request G0", numberOfBiSimCheckingRequestG0));
		timeInfos.add(new InfoHolder(39, "Number Of Real BiSim Checking G0", numberOfRealBiSimCheckingG0));
		...
		timeInfos.add(new InfoHolder(43, "Creation/Checking Of Dangling Nodes G0", danglingCreationDurationG0));
		timeInfos.add(new InfoHolder(44, "Number of Dangling Nodes G0", numberOfDanglingG0));

		timeInfos.add(new InfoHolder(45, "BiSim Checking Time Inc", prefixTree.getDurationOfBiSimChecking()));
		timeInfos.add(new InfoHolder(46, "Number Of BiSim Checking Request Inc",
				prefixTree.getNumberOfBiSimCheckingRequest()));
		timeInfos.add(
				new InfoHolder(47, "Number Of Real BiSim Checking Inc", prefixTree.getNumberOfRealBiSimChecking()));
		timeInfos.add(new InfoHolder(48, "Creation Of New PrefixTree Node Inc",
				prefixTree.getDurationOfNewPrefixTreeGeneration()));
		timeInfos.add(new InfoHolder(49, "Num. of support computations Inc", prefixTree.getNumberOfComputeSupport()));
		timeInfos.add(new InfoHolder(50, "Support computational time Inc", prefixTree.getDurationOfComputeSupport()));
		timeInfos.add(new InfoHolder(51, "Creation/Checking Of Dangling Nodes Inc",
				(prefixTree.danglingCreationDuration - danglingCreationDurationG0)));
		timeInfos.add(
				new InfoHolder(52, "Number of Dangling Nodes Inc", (prefixTree.numberOfDangling - numberOfDanglingG0)));

		timeInfos.add(new InfoHolder(47, "Update neighborhood info", updateNeighborhoodDuration));

		timeInfos.add(new InfoHolder(48, "Exp > checkValidityAtLeastOneMatchForEachPatternNode",
				prefixTree.checkValidityAtLeastOneMatchForEachPatternNodeDuration));

		timeInfos.add(new InfoHolder(49, "Exp > processQueueNode", prefixTree.processQueueNodeDuration));
		timeInfos.add(new InfoHolder(50, "WindowSize", DummyProperties.WINDOW_SIZE));
		timeInfos.add(new InfoHolder(51, "WindowMode", DummyProperties.windowMode));
		timeInfos.add(new InfoHolder(52, "carryOverDuraion", carryOverDuraion));
		timeInfos.add(new InfoHolder(53, "winMineDuraion", winMineDuraion));
		
		TimeLogger.LogTime("disnBiSim_" + (DummyProperties.windowMode ? "Win" : "Inc") + "_"
	...
	public SnapshotTopkMonitoringBiSim(String[] args) throws Exception {
	...
	public SnapshotTopkMonitoringBiSim(String focusSetPath, int maxHops, int maxEdges, String dataGraphPath,
	...
	public static void main(String[] args) throws Exception {
		SnapshotTopkMonitoringBiSim snapshotTopkMonitoring = new SnapshotTopkMonitoringBiSim(args);
	...
	private void findAffectedParentPatternsForInsertAndAddNewUnexpandedMatch(BooleanInside booleanInsideForIns,
			...
			int destNodeId, RoaringBitmap srcRoaringBitmap, RoaringBitmap destRoaringBitmap, String destLabel) {
				...
				if (prefixTNodeData.getPatternGraph().edgeSet().size() < maxAllowedEdges) {
					for (PatternNode patternNode : prefixTNodeData.getMatchedNodes().getPatternNodeOfNeo4jNode()
						...
						if (prefixTNodeData.getStepsFromRootOfPatternNodes().get(patternNode) < maxAllowedHops) {
							...
							// because of a bug in mixed mode: if a
							// prefixTreeNode is not
							// ready yet (i.e. invalid)
							// it will lose the destNodeId information because
							// it cannot expand from this prefixTree node.
							// for (DefaultLabeledEdge e :
							// prefixTNodeData.getPatternGraph()
							// .outgoingEdgesOf(patternNode)) {
							//
							// PatternNode correspondingDestPatternNode =
							// prefixTNodeData.getPatternGraph()
							// .getEdgeTarget(e);
							// if
							// (correspondingDestPatternNode.getLabel().equals(destLabel))
							// {
							//// prefixTNodeData.addNewMatch(correspondingDestPatternNode,
							// destNodeId,
							//// prefixTree.getLabelAdjacencyIndexer());
							//
							// prefixTNodeData.getNewUnexpandedPatternsNodesOfNeo4jNodes()
							// .putIfAbsent(correspondingDestPatternNode, new
							// HashSet<Integer>());
							// prefixTNodeData.getNewUnexpandedPatternsNodesOfNeo4jNodes()
							// .get(correspondingDestPatternNode).add(destNodeId);
							//
							// prefixTNodeData.getNewUnexpandedNodesOfPatternNodes().putIfAbsent(destNodeId,
							// new HashSet<PatternNode>());
							// prefixTNodeData.getNewUnexpandedNodesOfPatternNodes().get(destNodeId)
							// .add(correspondingDestPatternNode);
							// }
							// }
	...
	private StreamEdge handlingDataStream(StreamEdge nextStreamEdge, IPrefixTree prefixTree, Node srcNode,
		...
		if (nextStreamEdge.isAdded()) {
			...
			if (existed) {
				...
				if (prefixTree.getLabelAdjacencyIndexer().relTypeOfSrcAndDest.get(srcNodeId) != null)
