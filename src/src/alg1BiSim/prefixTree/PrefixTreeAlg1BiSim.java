...
public class PrefixTreeAlg1BiSim implements IPrefixTree {
	SnapshotTopkMonitoringBiSim snapshotTopkMonitoring;
	...
	public PrefixTreeAlg1BiSim(SnapshotTopkMonitoringBiSim snapshotTopkMonitoring) throws Exception {
		...
		while (!traversalQueue.isEmpty()) {
			...
			try {
				...
				if (!tempProcessingNode.getData().isDanglingPattern()
						...
						&& tempProcessingNode.getData().getSourcePatternNode() != tempProcessingNode.getData()
								.getPatternRootNode()) {
	...
	private boolean createDanglingPrefixTreeNodesIfNeeded(PrefixTreeNode<IPrefixTreeNodeData> tempProcessingNode,
		...
		int minStepChangedPatternNode = Integer.MAX_VALUE;
		HashSet<PatternNode> minCorrespondingPatternNode = new HashSet<PatternNode>();

		// checking for neighbors if source node is the neighbor of focus
		Set<DefaultLabeledEdge> edges = tempProcessingNode.getData().getPatternGraph().getAllEdges(
				tempProcessingNode.getData().getPatternRootNode(), tempProcessingNode.getData().getSourcePatternNode());
		...
		if (edges.size() > 0) {
			if (tempProcessingNode.getData().getMatchedNodes().getDataGraphMatchNodeOfAbsPNode()
				...
				minCorrespondingPatternNode.add(tempProcessingNode.getData().getSourcePatternNode());
			}
		} else {

			AllDirectedPaths<PatternNode, DefaultLabeledEdge> allPaths = new AllDirectedPaths<PatternNode, DefaultLabeledEdge>(
					tempProcessingNode.getData().getPatternGraph());

			List<GraphPath<PatternNode, DefaultLabeledEdge>> focusToSourceNodePaths = allPaths.getAllPaths(
					tempProcessingNode.getData().getPatternRootNode(),
					tempProcessingNode.getData().getSourcePatternNode(), false, maxAllowedHops);

			for (GraphPath<PatternNode, DefaultLabeledEdge> focusToSourceNodePath : focusToSourceNodePaths) {
				List<PatternNode> visitedPatternNodes = Graphs.getPathVertexList(focusToSourceNodePath);
				for (int i = 0; i < visitedPatternNodes.size(); i++) {
					if (!visitedPatternNodes.get(i).isFocus()) {
						if (tempProcessingNode.getData().getMatchedNodes().getDataGraphMatchNodeOfAbsPNode()
								.get(visitedPatternNodes.get(i)).size() != tempProcessingNode.getParent().getData()
										.getMatchedNodes().getDataGraphMatchNodeOfAbsPNode()
										.get(visitedPatternNodes.get(i)).size()) {
							minCorrespondingPatternNode.add(visitedPatternNodes.get(i));
							break;
						}
					}
				}
		...
		// if (tempProcessingNode.getData().getStepsFromRootOfPatternNodes()
		// .get(patternNode) <= minStepChangedPatternNode
		// &&
		// (tempProcessingNode.getData().getMatchedNodes().getDataGraphCandidateNodeOfAbsPNode()
		// .get(patternNode).size() !=
		// tempProcessingNode.getData().getMatchedNodes()
		// .getDataGraphMatchNodeOfAbsPNode().get(patternNode).size())) {
		// if (tempProcessingNode.getData().getStepsFromRootOfPatternNodes()
		// .get(patternNode) < minStepChangedPatternNode) {
		// minStepChangedPatternNode =
		// tempProcessingNode.getData().getStepsFromRootOfPatternNodes()
		// .get(patternNode);
		// minCorrespondingPatternNode.clear();
		// }
		//
		// minCorrespondingPatternNode.add(patternNode);
		//
		// }
		// }
		// }

		for (PatternNode minPatternNode : minCorrespondingPatternNode) {
			...
			for (DefaultLabeledEdge incomingE : tempProcessingNode.getData().getPatternGraph()
					.incomingEdgesOf(minPatternNode)) {

				PatternNode prevMinPatternNode = tempProcessingNode.getData().getPatternGraph()
						.getEdgeSource(incomingE);
					...
					if (dest != minPatternNode) {
						if (dest.getType().equals(minPatternNode.getType())
								&& tempProcessingNode.getData().getStepsFromRootOfPatternNodes().get(dest)
										.equals(tempProcessingNode.getData().getStepsFromRootOfPatternNodes()
												.get(minPatternNode))
								&& tempProcessingNode.getData().getIncomingRelTypesOfPatternNodes().get(minPatternNode)
										.equals(tempProcessingNode.getData().getIncomingRelTypesOfPatternNodes()
												.get(dest))) {
				...
				if (remainingNodeIds.size() > 0) {
					...
					for (Integer nodeId : tempProcessingNode.getData().getMatchedNodes()
						...
						if (labelAdjacencyIndexer.distinctNodesOfDHopsAway.get(nodeId).containsKey(newNodeType)
							...
							max = Math.max(labelAdjacencyIndexer.distinctNodesOfDHopsAway.get(nodeId).get(newNodeType)
									.get(Indexer.AFTER).size(), max);
					}
					...
					if (max > tempProcessingNode.getData().getFrequencyOfNextNeighborOfSameType()
						...
						if (oldDanglingPatternNode != null) {
							this.labelAdjacencyIndexer.candidateSetOfAPatternNode.get(oldDanglingPatternNode)
									.addAll(remainingNodeIds);

							for (Integer nodeId : remainingNodeIds) {
								tempProcessingNode.getData().getMatchedNodes().getPatternNodeOfNeo4jNode()
										.putIfAbsent(nodeId, new HashSet<PatternNode>());
							...
							if (DummyProperties.debugMode) {
								System.out.println(
										"adding some new matches in dangling proc: " + tempProcessingNode.getData());
							...
							SetView<Integer> intersectionOfTargetNodeAndRemainingIds = Sets
									.intersection(remainingNodeIds,
											tempProcessingNode.getData().getMatchedNodes()
							...
							// if there is any intersection we should create a
							// pattern for that
							if (intersectionOfTargetNodeAndRemainingIds.size() > 0) {
								...
								for (PatternNode patternNode : tempProcessingNode.getData().getPatternGraph()
										.vertexSet()) {
									newerPatternGraph.addVertex(patternNode);
								}

								for (DefaultLabeledEdge e : tempProcessingNode.getData().getPatternGraph().edgeSet()) {
									newerPatternGraph.addEdge(newerPatternGraph.getEdgeSource(e),
											newerPatternGraph.getEdgeTarget(e), e);
								}
								...
								newerPatternGraph.addEdge(
										tempProcessingNode.getData().getPatternGraph().getEdgeSource(incomingE),
										danglingPatternNode, new DefaultLabeledEdge(incomingE.getType()));
									...
									if (this.labelAdjacencyIndexer.prefixTreeNodesOfALevel
										...
										for (PrefixTreeNode<IPrefixTreeNodeData> prefixTreeSibling : this.labelAdjacencyIndexer.prefixTreeNodesOfALevel
											...
											if (DualSimulationHandler.isBiDualSimulated(
												...
												if (DummyProperties.debugMode) {
													System.out.println(
															"DANGLING: simultanous siblings: add node link from "
																	+ tempProcessingNode.getData()
										...
										for (PrefixTreeNode<IPrefixTreeNodeData> prefixTreeSibling : this.labelAdjacencyIndexer.prefixTreeNodesOfALevel
											...
											if (DualSimulationHandler.preBiSimChecking(
													...
													&& (dualBiSimMap = DualSimulationHandler.getBiSimMapIfAny(
															newPatternGraphAndPreMatches.patternGraph,
															prefixTreeSibling, this)) != null) {

												for (PatternNode danglingPTN : dualBiSimMap
														.get(newPatternGraphAndPreMatches.danglingPatternNode)) {
													prefixTreeSibling.getData().addNewMatchForUpdateDangling(
															tempProcessingNode, danglingPTN, remainingNodeIds,
								...
								if (!hadBiSimulInTheLevel) {
									...
									if (DummyProperties.debugMode) {
										System.out.println("new dangling child: "
												+ tempProcessingNode.getData().getPatternGraph()
														.getEdgeSource(incomingE)
												+ " -> " + newPatternGraphAndPreMatches.danglingPatternNode
												+ " rem. nodeIds:" + newPatternGraphAndPreMatches.preMatches);
										System.out.println(" from " + tempProcessingNode.getData());
									}

									numberOfDangling++;

									createNewPrefixTreeNode(tempProcessingNode,
											newPatternGraphAndPreMatches.patternGraph, prevMinPatternNode,
											newPatternGraphAndPreMatches.danglingPatternNode,
											newPatternGraphAndPreMatches.preMatches, newCreatedOrTouchedPTNodes,
											incomingE.getType(), tempProcessingNode.getData()
													.getStepsFromRootOfPatternNodes().get(minPatternNode),
											snapshot, null, true);
									isCreated = true;
		...
		for (PatternNode srcPatternNode : tempProcessingNodeData.getPatternGraph().vertexSet()) {
			...
			if (DummyProperties.debugMode) {
				System.out.println("srcAbstractPatternNode:" + srcPatternNode.getType() + "" + srcPatternNode.isFocus()
						+ " hashCode:" + srcPatternNode.hashCode());
			}
			...
			for (Integer srcDataGpNodeId : tempProcessingNodeData.getMatchedNodes().getDataGraphMatchNodeOfAbsPNode()
				...
				for (String otherNodeLabelRelType : labelAdjacencyIndexer.distinctNodesOfDHopsAway.get(srcDataGpNodeId)
					...
					for (Integer newNodeId : sameLabelNeighborNodes) {
						...
						Integer destStepsFromRoot = tempProcessingNodeData.getStepsFromRootOfPatternNodes()
								.get(srcPatternNode) + 1;
						if (DummyProperties.debugMode) {
							System.out.println(" srcNodeId:" + srcDataGpNodeId + ", newNodeId:" + newNodeId);
						}
						...
						getDestPatternNodeAndCheckForGoBackToPrev(destPatternNodes, tempProcessingNodeData,
								srcPatternNode, srcDataGpNodeId, newNodeId, destLabel, relationshipType,
								destStepsFromRoot);
						...
						if (tempProcessingNodeData.getFrequencyOfNextNeighborOfSameType().containsKey(srcPatternNode)
							...
							if (!goBackToPrev && tempProcessingNodeData.getFrequencyOfNextNeighborOfSameType()
									.get(srcPatternNode).get(otherNodeLabelRelType) > 0)
								continue;
						...
						for (PatternNode destPatternNode : destPatternNodes) {
							...
							if (goBackToPrev) {
								...
								for (DefaultLabeledEdge e : tempProcessingNodeData.getPatternGraph()
									...
									if (tempProcessingNodeData.getPatternGraph().getEdgeSource(e).getLabel().equals(
											srcPatternNode.getLabel()) && e.getType().equals(relationshipType)) {
							...
							if (destInDegree > labelAdjacencyIndexer.dataGraphNodeInfos.get(newNodeId).inDegree) {
								if (DummyProperties.debugMode) {
									System.out.println(
											"cont: destInDegree:" + destInDegree + " > in degree in data graph:"
							...
							if (goBackToPrev) {
								...
								if (newlySeenPatternNodeForThisSrc.containsKey(
									...
									// that
									// prefix-tree node
									seenPrefixTreeNode = newlySeenPatternNodeForThisSrc.get(newNodeType
											+ DummyProperties.SEPARATOR_LABEL_AND_RELTYPE + relationshipType);
								}
							}
							...
							if (seenPrefixTreeNode != null) {
								...
								seenPatternNodeFromPreviousNodesForThisSrc.put(tempDestPatternNode, seenPrefixTreeNode);

								if (newCreatedOrTouchedPTNodes != null)
									newCreatedOrTouchedPTNodes
											.add(seenPrefixTreeNode.getData().getPatternPrefixTreeNodeIndex());
							} else {
								...
								if (tempProcessingNodeData.getPrefixTreeMode() == PrefixTreeMode.BATCH) {
									if (this.labelAdjacencyIndexer.prefixTreeNodesOfALevel
										...
										for (PrefixTreeNode<IPrefixTreeNodeData> prefixTreeSibling : this.labelAdjacencyIndexer.prefixTreeNodesOfALevel
											...
											if (DualSimulationHandler.isBiDualSimulated(newPatternGraph,
												...
												if (DummyProperties.debugMode) {
													System.out
															.println("BATCH: simultanous siblings: add node link from "
																	+ tempProcessingNode.getData()
																			.getPatternPrefixTreeNodeIndex()
																	+ " to " + prefixTreeSibling.getData()
																			.getPatternPrefixTreeNodeIndex());
									...
									if (!itWasBisimulated && this.labelAdjacencyIndexer.prefixTreeNodesOfALevel
										...
										for (PrefixTreeNode<IPrefixTreeNodeData> prefixTreeNode : this.labelAdjacencyIndexer.prefixTreeNodesOfALevel
											...
											if (DualSimulationHandler.isBiDualSimulated(newPatternGraph, prefixTreeNode,
													this)) {
									...
									for (PrefixTreeNode<IPrefixTreeNodeData> child : tempProcessingNode.getChildren()) {
										...
										if (DualSimulationHandler.preBiSimChecking(child.getData().getPatternGraph(),
												newPatternGraph, this)
												&& (dualBiSimMap = DualSimulationHandler
														.getBiSimMapIfAny(newPatternGraph, child, this)) != null) {
											...
											for (PatternNode destPtn : dualBiSimMap.get(destPatternNode)) {
												...
												child.getData().addNewMatchForUpdate(tempProcessingNode,
														tempSrcPatternNode, srcDataGpNodeId, destPtn, newNodeId,
														this.labelAdjacencyIndexer);
											}
											...
											newCreatedOrTouchedPTNodes
													.add(child.getData().getPatternPrefixTreeNodeIndex());
											break;
										}
									}
									...
									if (!itWasBisimulated && tempProcessingNode.getLinkedNodes() != null) {
										...
										for (PrefixTreeNode<IPrefixTreeNodeData> child : tempProcessingNode
											...
											if (DualSimulationHandler.preBiSimChecking(
													child.getData().getPatternGraph(), newPatternGraph, this)
													&& (dualBiSimMap = DualSimulationHandler
															.getBiSimMapIfAny(newPatternGraph, child, this)) != null) {
									...
									if (!itWasBisimulated && this.labelAdjacencyIndexer.prefixTreeNodesOfALevel
										...
										for (PrefixTreeNode<IPrefixTreeNodeData> prefixTreeNodeByLevel : this.labelAdjacencyIndexer.prefixTreeNodesOfALevel
											...
											HashMap<PatternNode, HashSet<PatternNode>> dualBiSimMap;
											if (DualSimulationHandler.preBiSimChecking(
													prefixTreeNodeByLevel.getData().getPatternGraph(), newPatternGraph,
													this)
													&& (dualBiSimMap = DualSimulationHandler.getBiSimMapIfAny(
															newPatternGraph, prefixTreeNodeByLevel, this)) != null) {
												itWasBisimulated = true;

												PatternNode tempSrcPatternNode = null;
												tempSrcPatternNode = dualBiSimMap.get(srcPatternNode).iterator().next();
												for (PatternNode destPtn : dualBiSimMap.get(destPatternNode)) {
													prefixTreeNodeByLevel.getData().addNewMatchForUpdate(
															tempProcessingNode, tempSrcPatternNode, srcDataGpNodeId,
															destPtn, newNodeId, this.labelAdjacencyIndexer);
												}

												addIncNodeLink(tempProcessingNode, prefixTreeNodeByLevel);

												if (DummyProperties.debugMode)
													System.out.println("new match for a SGI node in INC mode!");

												traversalQueue.add(prefixTreeNodeByLevel);
												newCreatedOrTouchedPTNodes.add(prefixTreeNodeByLevel.getData()
														.getPatternPrefixTreeNodeIndex());
								...
								if (!itWasBisimulated) {
									...
									PrefixTreeNode<IPrefixTreeNodeData> newChild = createNewPrefixTreeNode(
											tempProcessingNode, newPatternGraph, srcPatternNode, destPatternNode,
											newNodeIds, newCreatedOrTouchedPTNodes, relationshipType, destStepsFromRoot,
											snapshot, newChildrenOfTheSrc, false);
	...
	// public void expandForNewInsertedEdge(Node srcNode, Integer srcNodeId,
	// String srcLabel, Node destNode,
	// Integer destNodeId, String destLabel, boolean srcIsAFocus, boolean
	// destIsAFocus,
	// PrefixTreeNode<IPrefixTreeNodeData> prefixTreeNode, HashSet<Integer>
	// newCreatedOrTouchedPTNodes,
	// int snapshot, double threshold,
	// MinMaxPriorityQueue<PrefixTreeNode<IPrefixTreeNodeData>>
	// topKFrequentPatterns, String relationshipType)
	// throws Exception {
	//
	...
	// if (DummyProperties.debugMode) {
	// System.out.println("srcPatternNode: " + srcPatternNode + "srcNodeId: " +
	// srcNodeId + ", destNodeId: "
	// + destNodeId + ", destLabel:" + destLabel);
	// }
	//
	// HashMap<PrefixTreeNode<IPrefixTreeNodeData>, ArrayList<PatternNode>>
	// thePTNodeBaseOnTheNewEdge = new
	// HashMap<PrefixTreeNode<IPrefixTreeNodeData>, ArrayList<PatternNode>>();
	//
	// if (DummyProperties.debugMode) {
	// System.out.println("incremental srcAbstractPatternNode : " +
	// srcPatternNode.getType() + ""
	// + srcPatternNode.isFocus());
	// }
	//
	...
	// // patterns
	// if
	// (prefixTreeNode.getData().getStepsFromRootOfPatternNodes().get(srcPatternNode)
	// >= maxAllowedHops)
	// // throw new Exception(srcPatternNode + " >= " +
	// // maxAllowedHops);
	// continue;
	//
	// Integer tempDestStepsFromRoot =
	...
	// srcNodeId, destNodeId, destLabel, relationshipType,
	// tempDestStepsFromRoot);
	//
	...
	// // allowed to!
	// HashMap<String, Integer> frequencyOfSameOutNeighborType =
	// getFrequencyOfSameOutNeighborType(
	// prefixTreeNode.getData(), srcPatternNode);
	//
	// HashSet<Integer> sameLabelNeighborNodes =
	// labelAdjacencyIndexer.distinctNodesOfDHopsAway
	// .get(srcNodeId)
	...
	// .containsKey(destLabel + DummyProperties.SEPARATOR_LABEL_AND_RELTYPE +
	// relationshipType)
	// && frequencyOfSameOutNeighborType
	// .get(destLabel + DummyProperties.SEPARATOR_LABEL_AND_RELTYPE
	...
	//
	// thePTNodeBaseOnTheNewEdge.putIfAbsent(prefixTreeNode, new
	...
	// for (PrefixTreeNode<IPrefixTreeNodeData> childPTNode :
	// prefixTreeNode.getChildren()) {
	// // from same source go the same destLabel and type
	// if (sameChildLinkedPattern(childPTNode, srcPatternNode,
	// tempDestPatternNode,
	...
	// // we should add the new potential matches there.
	// for (PrefixTreeNode<IPrefixTreeNodeData> linkedNode :
	// prefixTreeNode.getLinkedNodes()) {
	// if (sameChildLinkedPattern(linkedNode, srcPatternNode,
	// tempDestPatternNode,
	// tempDestStepsFromRoot, relationshipType)) {
	...
	// newerPatternGraph = null;
	//
	// if (thePTNodeBaseOnTheNewEdge.isEmpty()) {
	// newerPatternGraph = createNewPatternFromParentPattern(prefixTreeNode,
	// srcPatternNode,
	...
	// if (DummyProperties.debugMode) {
	// System.out.println(
	// "the pattern is found in the newly touched or created patterns so we
	...
	//
	// Iterator<GraphMapping<PatternNode, DefaultLabeledEdge>> mappingItr =
	// iso.getMappings();
	...
	// tempDestPatternNodeIso = map.getVertexCorrespondence(tempDestPatternNode,
	// true);
	// break;
	// }
	...
	// if (!thePTNodeBaseOnTheNewEdge.isEmpty()) {
	// for (PrefixTreeNode<IPrefixTreeNodeData> thePTNode :
	// thePTNodeBaseOnTheNewEdge.keySet()) {
	...
	// }
	// if
	// (!newCreatedOrTouchedPTNodes.contains(thePTNode.getData().getPatternPrefixTreeNodeIndex()))
	// {
	// for (PatternNode destPatternNode :
	// thePTNodeBaseOnTheNewEdge.get(thePTNode)) {
	//
	// // if
	// // (prefixTreeNode.getData().getPatternPrefixTreeNodeIndex()
	// // == 199) {
	// // System.out.println();
	// // }
	// thePTNode.getData().addNewMatchForUpdate(prefixTreeNode, srcPatternNode,
	// srcNodeId,
	...
	//
	// HashSet<Integer> newNodeIds = new HashSet<Integer>();
	// newNodeIds.add(destNodeId);
	//
	// thePTNodeBaseOnTheNewEdge.put(createNewPrefixTreeNode(prefixTreeNode,
	// newerPatternGraph,
	// srcPatternNode, tempDestPatternNode, newNodeIds,
	// newCreatedOrTouchedPTNodes,
	// relationshipType, tempDestStepsFromRoot, snapshot, null, false), null);
	// newPrefixTreeNodeIsCreated = true;
	// if (DummyProperties.debugMode)
	// System.out.println("the pattern isn't found, so a new pattern is
	// created!");
	// }
	//
	...
	//
	// if (DummyProperties.debugMode)
	// System.out.println("thePTNodeBaseOnTheNewEdge size: " +
	// thePTNodeBaseOnTheNewEdge.size());
	//
	// for (PrefixTreeNode<IPrefixTreeNodeData> thePTNode :
	// thePTNodeBaseOnTheNewEdge.keySet()) {
	...
	// // continue;
	// if (DummyProperties.debugMode)
	// System.out.println("start from here: the prifixTree node related to the
	// new added edge: "
	...
	//
	// generatePrefixTreeFromHere(topKFrequentPatterns,
	// newCreatedOrTouchedPTNodes,
	...
	public void shrinkForNewDeletedEdge(Node srcNode, Node destNode, boolean srcIsAFocus, boolean destIsAFocus,
		...
		while (!shrinkageQueue.isEmpty()) {
			...
			if (DummyProperties.debugMode) {
				System.out.println("DEL-Affected: Before remove: " + sourceLabel + srcNodeId + "->" + destLabel
						+ destNodeId + ":" + thisNode.getData());
			}
