...
public class PrefixTreeOptBiSimSGI implements IPrefixTree {
	// SnapshotTopkMonitoringOptBiSimSGI snapshotTopkMonitoring;
	...
	public PrefixTreeOptBiSimSGI(String[] args) throws Exception {
	...
	// public PrefixTreeOptBiSimSGI(SnapshotTopkMonitoringOptBiSimSGI
	// snapshotTopkMonitoring) throws Exception {
	 ...
	 * @throws Exception
	 */
	public PrefixTreeOptBiSimSGI(String focusSetPath, int maxAllowedHops, int maxAllowedEdges,
	...
	public static void main(String[] args) throws Exception {
		...
		PrefixTreeOptBiSimSGI prefixTree = new PrefixTreeOptBiSimSGI(args);
	...
	private PrefixTreeNode<IPrefixTreeNodeData> initializePrefixTree() throws Exception {
		...
		IPrefixTreeNodeData emptyPTRootData = new PrefixTreeNodeDataOptBiSimSGI(focusLabelSet);
		...
		for (String focusLabel : allNodesOfFocusType.keySet()) {
			...
			IPrefixTreeNodeData firstLevelChildData = new PrefixTreeNodeDataOptBiSimSGI(focusNode, dgGraphMatchNodes,
	...
	public PrefixTreeNode<IPrefixTreeNodeData> generatePrefixTreeFromHere(
		...
		while (!traversalQueue.isEmpty()) {
			...
			try {
				...
				if (!tempProcessingNode.getData().isDanglingPattern()
						&& tempProcessingNode.getData().isMaximalFrequent()
								...
								if (tempProcessingNode.getData().getPrefixTreeMode() == PrefixTreeMode.BATCH) {
									if (this.labelAdjacencyIndexer.prefixTreeNodesOfALevel
										...
										for (PrefixTreeNode<IPrefixTreeNodeData> prefixTreeSibling : this.labelAdjacencyIndexer.prefixTreeNodesOfALevel
											...
											if (getIsomorphism(newPatternGraphAndPreMatches.patternGraph,
						...
						for (PatternNode destPatternNode : destPatternNodes) {
							...
							if (seenPrefixTreeNode != null) {
								...
								if (tempProcessingNodeData.getPrefixTreeMode() == PrefixTreeMode.BATCH) {
									if (this.labelAdjacencyIndexer.prefixTreeNodesOfALevel
											.containsKey(prefixTreeProcessingLevel)) {
										for (PrefixTreeNode<IPrefixTreeNodeData> prefixTreeSibling : this.labelAdjacencyIndexer.prefixTreeNodesOfALevel
											...
											// System.out.println();
											// }
											if (getIsomorphism(newPatternGraph,
													prefixTreeSibling.getData().getPatternGraph())
															.isomorphismExists()) {
										...
										if (!itWasIsomorphic && this.labelAdjacencyIndexer.prefixTreeNodesOfALevel
									...
									throw new Exception(" not a right place!");
								}
								if (!itWasIsomorphic) {
	...
	private PrefixTreeNode<IPrefixTreeNodeData> createNewPrefixTreeNode(
		...
		IPrefixTreeNodeData prefixTreeNodeData = new PrefixTreeNodeDataOptBiSimSGI(newPatternGraph,
	...
	private void printTreeDualSim(PrefixTreeNode<IPrefixTreeNodeData> rootNode, GraphDatabaseService graphdb,
			PrefixTreeOptBiSimSGI prefixTree) {
		Queue<PrefixTreeNode<IPrefixTreeNodeData>> bfsQueue = new LinkedList<PrefixTreeNode<IPrefixTreeNodeData>>();
		bfsQueue.add(rootNode);
