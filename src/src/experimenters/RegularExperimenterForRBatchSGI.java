...
public class RegularExperimenterForRBatchSGI {
	...
	public static void main(String[] args) throws Exception {
		...
		File foutRFA = new File("reasonableFocusesAutomat.txt");
		FileOutputStream fosRFA = new FileOutputStream(foutRFA);

		BufferedWriter bwRFA = new BufferedWriter(new OutputStreamWriter(fosRFA));
				...
				for (int e : maxAllowedEdges) {
					for (double t : thresholds) {
						...
						for (int n = startingInterval; n < numberOfIntervals; n++) {
							...
							// Batch SGI
							// for (int exp = 0; exp < numberOfSameExperiments;
							// exp++) {
							// StaticGraphTopK staticGraphTopK = new
							// StaticGraphTopK("focusSet.txt", h, e,
							// dataGraphPath, debugMode, k, t,
							...
							// // }
							//
							// System.out.println("StaticGraphTopK: exp " + exp
							// + " focus was " + line + ", h:" + h
							// + " , e:" + e + " , k:" + k + ", threshold:" + t
							// + " in interval: " + n);
							//
							...
							for (int exp = 0; exp < numberOfSameExperiments; exp++) {
								StaticGraphTopKBiSimOpt staticGraphTopK = new StaticGraphTopKBiSimOpt("focusSet.txt", h,
										e, dataGraphPath, debugMode, k, t, DummyProperties.windowMode,
										DummyProperties.WINDOW_SIZE);
								// double startTime = System.nanoTime();
								staticGraphTopK.findStaticTopK();
								...
								System.out.println("StaticGraphTopKBiSimOpt: exp " + exp + " focus was " + line + ", h:"
							...
							// Batch BiSim SGI
							for (int exp = 0; exp < numberOfSameExperiments; exp++) {
								StaticGraphTopKBiSimOptSGI staticGraphTopK = new StaticGraphTopKBiSimOptSGI(
								...
								System.out.println("StaticGraphTopKBiSimOptSGI: exp " + exp + " focus was " + line
										+ ", h:" + h + " , e:" + e + " , k:" + k + ", threshold:" + t + " in interval: "
										+ n);
