						...
						for (int k : ks) {
							...
							for (int n = 0; n < numberOfIntervals; n++) {
								...
								for (int exp = 0; exp < numberOfSameExperiments; exp++) {
									...
									prepareDateSetBasedOnStream(newDGPath, deltaEFileOrFiles, dateFormat);
									StaticGraphTopK staticGraphTopK = new StaticGraphTopK("focusSet.txt", h, e,
											newDGPath.toString(), debugMode, k, t, DummyProperties.windowMode,
...
//								}
								...
								for (int exp = 0; exp < numberOfSameExperiments; exp++) {
									...
									SnapshotTopkMonitoring snapshotTopkMonitoring = new SnapshotTopkMonitoring(
											"focusSet.txt", h, e, newDGPath.toString(), debugMode, k, t,
											deltaEFileOrFiles, numberOfTransactionInASnapshot, numberOfSnapshots,
											dateFormat, numberOfIgnoranceInitEdges, DummyProperties.windowMode,
											DummyProperties.WINDOW_SIZE);

									snapshotTopkMonitoring.snapshotTopkMonitor();

									System.out.println("Inc: exp " + exp + " focus was " + line + ", h:" + h + " , e:"
	...
	private static void prepareDateSetBasedOnStream(Path newDGPath, String deltaEFileOrFiles, String dateFormat)
			throws Exception {
		...
		while (true) {
			...
			int srcNodeId = (int) nextStreamEdge.getSourceNode();
			int destNodeId = (int) nextStreamEdge.getDestinationNode();
			...
			if (nextStreamEdge.isAdded()) {
				boolean thereWereThisBefore = false;
				for (Relationship rel : srcNode.getRelationships(Direction.OUTGOING,
					...
					if (rel.getStartNode().getId() == srcNodeId && rel.getEndNode().getId() == destNodeId) {
						thereWereThisBefore = true;
					}
				}
				if (!thereWereThisBefore)
					srcNode.createRelationshipTo(destNode, RelationshipType.withName(relType));
			} else {
				for (Relationship rel : srcNode.getRelationships(Direction.OUTGOING,
						RelationshipType.withName(relType))) {
