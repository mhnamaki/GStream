...
public class RegularExperimenterForAllMemory {
	...
	public static void main(String[] args) throws Exception {
		...
		File foutMem = new File("memoryAnalysis.txt");
		FileOutputStream fosMem = new FileOutputStream(foutMem);

		BufferedWriter bwMemory = new BufferedWriter(new OutputStreamWriter(fosMem));
		bwMemory.write("alg, line, h, e, k, t, dataGraphPath, n, experimenter, patternTree, indexer");
		bwMemory.newLine();
					...
					for (double t : thresholds) {
						for (int k : ks) {
							...
							for (int exp = 0; exp < numberOfSameExperiments; exp++) {
								...
								System.out.println("Static: exp " + exp + " focus was " + line + ", h:" + h + " , e:"
										+ e + " , k:" + k + ", threshold:" + t + " in interval: " + n);

							
								writeMemAnalysis("Batch", bwMemory, line, h, e, k, t, dataGraphPath, n,
										meter.measureDeep(staticGraphTopK),
										meter.measureDeep(staticGraphTopK.prefixTree),
										meter.measureDeep(staticGraphTopK.prefixTree.labelAdjacencyIndexer));
							...
							for (int exp = 0; exp < numberOfSameExperiments; exp++) {
								...
								System.out.println("Static Bi Sim Opt: exp " + exp + " focus was " + line + ", h:" + h
										+ " , e:" + e + " , k:" + k + ", threshold:" + t + " in interval: " + n);

								writeMemAnalysis("rBatch", bwMemory, line, h, e, k, t, dataGraphPath, n,
										meter.measureDeep(staticGraphTopKBiSimOpt),
										meter.measureDeep(staticGraphTopKBiSimOpt.prefixTree),
										meter.measureDeep(staticGraphTopKBiSimOpt.prefixTree.labelAdjacencyIndexer));
	...
	private static void writeMemAnalysis(String algName, BufferedWriter bwMemory, String line, int h, int e, int k,
			double t, String dataGraphPath, int n, long experimenter, long patternTree, long indexer) throws Exception {

		bwMemory.write(algName + "," + line + "," + h + "," + e + "," + k + "," + t + "," + dataGraphPath + "," + n
				+ "," + experimenter + "," + (patternTree - indexer) + "," + indexer);
		bwMemory.newLine();
		bwMemory.flush();
	}
