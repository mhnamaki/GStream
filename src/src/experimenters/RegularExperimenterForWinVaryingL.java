...
public class RegularExperimenterForWinVaryingL {
	...
	public static void main(String[] args) throws Exception {
		...
		while ((line = br.readLine()) != null) {
			...
			for (Integer startingSnap : windowSizeLArr) {
				startingSnapshotBasedOnWindowSize = startingSnap - 1;
				...
				for (int h : maxAllowedHops) {
					for (int e : maxAllowedEdges) {
						for (double t : thresholds) {
							...
							dataGraphPath = getNextDataGraphPath(dataGraphPath, streamMode, interval,
									startingSnapshotBasedOnWindowSize);

							System.out.println("dataGraphPath: " + dataGraphPath);
							numberOfIgnoranceInitEdges = 70000;
							...
							for (int exp = 0; exp < numberOfSameExperiments; exp++) {
								...
								System.out.println("WinBatch Static: exp " + exp + " focus was " + line + ", h:" + h
										+ " , e:" + e + " , k:" + k + ", threshold:" + t + " in interval: " + 0);
							...
							for (int exp = 0; exp < numberOfSameExperiments; exp++) {
								...
								SnapshotTopkMonitoring snapshotTopkMonitoring = new SnapshotTopkMonitoring(
										...
										numberOfIgnoranceInitEdges, windowMode, windowSizeLInt);

								snapshotTopkMonitoring.snapshotTopkMonitor();

								System.out.println("WinDisn : exp " + exp + " focus  was " + line + ", h:" + h + " , e:"
										+ e + " , k:" + k + ", threshold:" + t + " in interval: " + 0);
							...
							for (int exp = 0; exp < numberOfSameExperiments; exp++) {
								...
								SnapshotTopkMonitoringOptBiSim snapshotTopkMonitoringOptBiSim = new SnapshotTopkMonitoringOptBiSim(
										"focusSet.txt", h, e, newDGPath.toString(), debugMode, k, t, deltaEFileOrFiles,
										numberOfTransactionInASnapshot, numberOfSnapshots, dateFormat,
										numberOfIgnoranceInitEdges, edgeStreamCacheCapacity, windowMode,
										windowSizeLInt);

								snapshotTopkMonitoringOptBiSim.snapshotTopkMonitor();

								System.out.println("WinDis+: exp " + exp + " focus was " + line + ", h:" + h + " , e:"
										+ e + " , k:" + k + ", threshold:" + t + " in interval: " + 0);
