...
import src.IsomorphismTest.IsoTester;
...
public class PrefixTreeSubOpt implements IPrefixTree {
	SnapshotTopkMonitoringSubOpt snapshotTopkMonitoring;
	...
	public PrefixTreeSubOpt(String[] args) throws Exception {
	...
	public PrefixTreeSubOpt(SnapshotTopkMonitoringSubOpt snapshotTopkMonitoring) throws Exception {
	 ...
	 * @throws Exception
	 */
	public PrefixTreeSubOpt(String focusSetPath, int maxAllowedHops, int maxAllowedEdges,
			GraphDatabaseService dataGraph, boolean debugMode, Bitmap bitMap) throws Exception {
	...
	public static void main(String[] args) throws Exception {
		...
		PrefixTreeSubOpt prefixTree = new PrefixTreeSubOpt(args);
	...
	private PrefixTreeNode<IPrefixTreeNodeData> initializePrefixTree() throws Exception {
		...
		IPrefixTreeNodeData emptyPTRootData = new PrefixTreeNodeDataSubOpt(focusLabelSet);
		...
		for (String focusLabel : allNodesOfFocusType.keySet()) {
			...
			IPrefixTreeNodeData firstLevelChildData = new PrefixTreeNodeDataSubOpt(focusNode, dgGraphMatchNodes,
		...
		while (!traversalQueue.isEmpty()) {
			...
			try {
				...
				if (wasMFP || wasMIP || !tempProcessingNode.getData().isVisited()
					...
					IsoTester.computeSGISupport(dataGraph, tempProcessingNode, snapshot, this);
	...
	public PrefixTreeNode<IPrefixTreeNodeData> generatePrefixTreeForG0() throws Exception {
		...
		while (!traversalQueue.isEmpty()) {
			...
			try {
				...
				//IsoTester.computeSGISupport(dataGraph, tempProcessingNode, 0, this);
	...
	private PrefixTreeNode<IPrefixTreeNodeData> createNewPrefixTreeNode(
		...
		IPrefixTreeNodeData prefixTreeNodeData = new PrefixTreeNodeDataSubOpt(newPatternGraph,
	...
	private void updateDeleteAffectedPrefixTreeNode(PrefixTreeNode<IPrefixTreeNodeData> thisNode,
		...
		if (wasMFP) {
			...
			IsoTester.computeSGISupport(dataGraph, thisNode, snapshot, this);
