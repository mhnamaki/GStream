...
public class PrefixTreeAlg1 implements IPrefixTree {
	...
	public PrefixTreeAlg1(String[] args) throws Exception {
	...
	public PrefixTreeAlg1(SnapshotTopkMonitoring snapshotTopkMonitoring) throws Exception {
	 ...
	 * @throws Exception
	 */
	public PrefixTreeAlg1(String focusSetPath, int maxAllowedHops, int maxAllowedEdges, GraphDatabaseService dataGraph,
			boolean debugMode, Bitmap bitMap,
			MinMaxPriorityQueue<PrefixTreeNode<IPrefixTreeNodeData>> topKFrequentPatterns, double threshold)
			throws Exception {
	...
	public static void main(String[] args) throws Exception {
		...
		PrefixTreeAlg1 prefixTree = new PrefixTreeAlg1(args);
	...
	private PrefixTreeNode<IPrefixTreeNodeData> initializePrefixTree() throws Exception {
		...
		IPrefixTreeNodeData emptyPTRootData = new PrefixTreeNodeDataAlg1(focusLabelSet);
		...
		for (String focusLabel : allNodesOfFocusType.keySet()) {
			...
			IPrefixTreeNodeData firstLevelChildData = new PrefixTreeNodeDataAlg1(focusNode, dgGraphMatchNodes,
					...
					if (!tempProcessingNode.getData().getFoundAllFocuses()) {
						...
						if ((labelAdjacencyIndexer.dataGraphNodeInfos
							...
							for (String otherNodeLabelRelType : labelAdjacencyIndexer.distinctNodesOfDHopsAway
								...
								// if (prefixTreeNodeIndex.get(7) != null) {
								// System.out.println("repeating\n" +
								// prefixTreeNodeIndex.get(7).getData().getMappedGraphString());
								// }
							}
						}
	...
	private void expandToFocusesFirst(PrefixTreeNode<IPrefixTreeNodeData> tempProcessingNode,
			IPrefixTreeNodeData tempProcessingNodeData, PatternNode srcPatternNode, int matchGraphIndex,
		...
		if (sameLabelNeighborNodes != null && sameLabelNeighborNodes.size() > 0) {
			if (!frequencyOfSameInNeighborType.containsKey(otherNodeLabelRelType)
				...
				for (Integer newNodeId : sameLabelNeighborNodes) {
					...
					for (DefaultLabeledEdge e : tempProcessingNodeData.getPatternGraph()
						...
						if (tempProcessingNode.getData().getMatchedNodes().getDataGraphMatchNodeOfAbsPNode()
								...
								&& (/*
									 ...
									 * .outDegreeOf( tempProcessingNodeData
									 * .getPatternGraph() .getEdgeTarget(e)) > 0
									 * ||
									 */ tempProcessingNode.getData().getMatchedNodes().getDataGraphMatchNodeOfAbsPNode()
										.get(tempProcessingNodeData.getPatternGraph().getEdgeSource(e)).size() == 1)) {
							expandedBefore = true;
							break;
						}
					...
					if (expandedBefore) {
						if (DummyProperties.debugMode)
							System.out.println("expanded before from " + srcDataGpNodeId + " to " + newNodeId);
						continue;
					}

					// goBackToPrev = false;
					ArrayList<PatternNode> destPatternNodes = new ArrayList<PatternNode>();
					getDestPatternNodeAndCheckForGoBackToPrevIncoming(destPatternNodes, tempProcessingNodeData,
							srcPatternNode, srcDataGpNodeId, newNodeId, destLabel, relationshipType, destStepsFromRoot);
					...
					for (int d = 0; d < destPatternNodes.size(); d++) {
						...
						if (goBackToPrev) {
							if (seenPatternNodeFromPreviousNodesForThisSrc.containsKey(destPatternNode)) {
								seenPrefixTreeNode = seenPatternNodeFromPreviousNodesForThisSrc.get(destPatternNode);
							}
						} else {
						...
						if (seenPrefixTreeNode != null) {
							...
							newPatternGraph.addEdge(destPatternNode, srcPatternNode,
							...
							for (DefaultLabeledEdge e : newPatternGraph.outgoingEdgesOf(destPatternNode)) {
							...
							addNewChildrenOrMatches(tempProcessingNode, tempProcessingNodeData, newPatternGraph,
									srcPatternNode, destPatternNode, srcDataGpNodeId, newNodeId,
				...
				for (int d = 0; d < destPatternNodes.size(); d++) {
					...
					if (goBackToPrev) {
						...
						if (newlySeenPatternNodeForThisSrc.containsKey(
							...
							// that
							// prefix-tree node
							seenPrefixTreeNode = newlySeenPatternNodeForThisSrc
	...
	private void outogingExpansion(PrefixTreeNode<IPrefixTreeNodeData> tempProcessingNode,
			IPrefixTreeNodeData tempProcessingNodeData, PatternNode srcPatternNode, int matchGraphIndex,
			HashMap<PatternNode, PrefixTreeNode<IPrefixTreeNodeData>> seenPatternNodeFromPreviousNodesForThisSrc,
			HashMap<String, PrefixTreeNode<IPrefixTreeNodeData>> newlySeenPatternNodeForThisSrc,
			HashSet<Integer> newCreatedOrTouchedPTNodes, Integer srcDataGpNodeId, String otherNodeLabelRelType,
				...
				for (Integer newNodeId : sameLabelNeighborNodes) {
					...
					for (DefaultLabeledEdge e : tempProcessingNodeData.getPatternGraph()
						...
						if (tempProcessingNode.getData().getMatchedNodes().getDataGraphMatchNodeOfAbsPNode()
								...
								&& (/*
									 ...
									 * ||
									 */ tempProcessingNode.getData().getMatchedNodes().getDataGraphMatchNodeOfAbsPNode()
										.get(tempProcessingNodeData.getPatternGraph().getEdgeTarget(e)).size() == 1)) {
							expandedBefore = true;
							break;
						}
					...
					for (int d = 0; d < destPatternNodes.size(); d++) {
						...
						if (seenPrefixTreeNode != null) {
							...
							for (DefaultLabeledEdge e : newPatternGraph.incomingEdgesOf(destPatternNode)) {
	...
	private void outogingExpansion2(PrefixTreeNode<IPrefixTreeNodeData> tempProcessingNode,
			IPrefixTreeNodeData tempProcessingNodeData, PatternNode srcPatternNode, int matchGraphIndex,
			HashMap<PatternNode, PrefixTreeNode<IPrefixTreeNodeData>> seenPatternNodeFromPreviousNodesForThisSrc,
			HashMap<String, PrefixTreeNode<IPrefixTreeNodeData>> newlySeenPatternNodeForThisSrc,
			HashSet<Integer> newCreatedOrTouchedPTNodes, Integer srcDataGpNodeId, String otherNodeLabelRelType,
	...
	private void addNewChildrenOrMatches(PrefixTreeNode<IPrefixTreeNodeData> tempProcessingNode,
		...
		if (tempProcessingNodeData.getPrefixTreeMode() == PrefixTreeMode.BATCH) {
			for (PrefixTreeNode<IPrefixTreeNodeData> prefixTreeSibling : sameLevelPrefixTreeNodes) {
				...
				// System.out.println();
				// }
				if (preIsoChecking(prefixTreeSibling.getData().getPatternGraph(), newPatternGraph)
						&& getIsomorphism(newPatternGraph, prefixTreeSibling.getData().getPatternGraph())
								.isomorphismExists()) {
					...
					// .contains(prefixTreeSibling))
					if (DummyProperties.debugMode) {
						System.out.println("BATCH: add node link from "
			...
			if (!itWasIsomorphic && this.getLabelAdjacencyIndexer().prefixTreeNodesOfALevel
					.containsKey(tempProcessingNode.getLevel() + 1)) {

				for (PrefixTreeNode<IPrefixTreeNodeData> prefixTreeNode : this
						.getLabelAdjacencyIndexer().prefixTreeNodesOfALevel.get(tempProcessingNode.getLevel() + 1)) {

					if (preIsoChecking(prefixTreeNode.getData().getPatternGraph(), newPatternGraph)
							&& getIsomorphism(newPatternGraph, prefixTreeNode.getData().getPatternGraph())
									.isomorphismExists()) {
			...
			for (PrefixTreeNode<IPrefixTreeNodeData> child : tempProcessingNode.getChildren()) {
				...
				if (preIsoChecking(child.getData().getPatternGraph(), newPatternGraph)
					...
					while (mappingItr.hasNext()) {
						...
						destPatternNodesFromIso.add(map.getVertexCorrespondence(destPatternNode, true));
					...
					// ArrayList<PatternNode>();
					//
					// fillDestPatternNodesCorrectly(child.getData(),
					// srcPatternNode,
					// destPatternNodes,
					// destPatternNode.getLabel(),
					// destPatternNode.isFocus(),
					// destStepsFromRoot,
					// relationshipType);
			...
			if (!itWasIsomorphic) {
				...
				for (Integer ptIndex : prefixTreeNodeIndex.keySet()) {
					if (prefixTreeNodeIndex.get(ptIndex).getLevel() == childLevel) {
						...
						if (tempProcessingNode.getChildren().contains(prefixTreeNodeIndex.get(ptIndex)))
							// it's
							// checked
							// before
							continue;
						...
						if (tempProcessingNode.getLinkedNodes() != null
								&& tempProcessingNode.getLinkedNodes().contains(prefixTreeNodeIndex.get(ptIndex))) {
							// it's
							// checked
							// before
							continue;
						...
						VF2GraphIsomorphismInspector<PatternNode, DefaultLabeledEdge> iso;
						if (preIsoChecking(prefixTreeNodeIndex.get(ptIndex).getData().getPatternGraph(),
								newPatternGraph)
							...
							for (PatternNode destPtn : destPatternNodesFromIso) {
								prefixTreeNodeIndex.get(ptIndex).getData().addNewMatchForUpdate(tempProcessingNode,
										tempSrcPatternNode, srcDataGpNodeId, destPtn, newNodeId,
										this.labelAdjacencyIndexer);
							}

							addIncNodeLink(tempProcessingNode, prefixTreeNodeIndex.get(ptIndex));

							if (DummyProperties.debugMode)
								System.out.println("new match for a SGI node in INC mode!");
							traversalQueue.add(prefixTreeNodeIndex.get(ptIndex));
							newCreatedOrTouchedPTNodes
									.add(prefixTreeNodeIndex.get(ptIndex).getData().getPatternPrefixTreeNodeIndex());
		...
		if (!itWasIsomorphic) {
			...
			PrefixTreeNode<IPrefixTreeNodeData> newChild = createNewPrefixTreeNode(tempProcessingNode, newPatternGraph,
					srcPatternNode, destPatternNode, srcDataGpNodeId, newNodeIds, newCreatedOrTouchedPTNodes,
					relationshipType, destStepsFromRoot,
					tempProcessingNode.getData().getPrefixTreeMode() == PrefixTreeMode.UPDATE ? !FRESH_SOURCE
							: FRESH_SOURCE,
					snapshot);
				...
				for (Integer ptIndex : prefixTreeNodeIndex.keySet()) {
					if (prefixTreeNodeIndex.get(ptIndex).getLevel() == childLevel) {
						...
						if (preIsoChecking(prefixTreeNodeIndex.get(ptIndex).getData().getPatternGraph(),
							...
							for (PatternNode destPtn : destPatternNodesFromIso) {
								prefixTreeNodeIndex.get(ptIndex).getData().addNewMatchSetForUpdate(tempProcessingNode,
	...
	private HashMap<String, Integer> getFrequencyOfSameInNeighborType(IPrefixTreeNodeData tempProcessingNodeData,
			PatternNode srcPatternNode) {
	...
	private PrefixTreeNode<IPrefixTreeNodeData> createNewPrefixTreeNode(
		...
		IPrefixTreeNodeData prefixTreeNodeData = new PrefixTreeNodeDataAlg1(newPatternGraph,
				...
				newNodeIds, numberOfPatternsInPrefixTree, relationshipType, destStepsFromRoot, freshSource, snapshot,
				this.labelAdjacencyIndexer);
		...
		if (DummyProperties.debugMode) {
			// if (newChild.getData().getPatternPrefixTreeNodeIndex() == 7) {
			// System.out.println("");
			// }
	...
	private void getDestPatternNodeAndCheckForGoBackToPrevIncoming(ArrayList<PatternNode> destPatternNodes,
	...
	// public void expandForNewInsertedEdgeIncoming(Node srcNode, Integer
	// srcNodeId, String srcLabel, Node destNode,
	...
	//
	// if
	// (prefixTreeNode.getData().getMatchedNodes().getPatternNodeOfNeo4jNode().containsKey(srcNodeId))
	// {
	// srcPatternNodes
	// .addAll(prefixTreeNode.getData().getMatchedNodes().getPatternNodeOfNeo4jNode().get(srcNodeId));
	// }
	//
	// if
	// (prefixTreeNode.getData().getMatchedNodes().getPatternNodeOfNeo4jNode().containsKey(destNodeId))
	// {
	// srcPatternNodes
	// .addAll(prefixTreeNode.getData().getMatchedNodes().getPatternNodeOfNeo4jNode().get(destNodeId));
	// }
	//
	...
	// newerPatternGraph,
	// srcPatternNode, tempDestPatternNode, srcNodeId, destNodeId,
	// newCreatedOrTouchedPTNodes,
	...
	// }
	//
	// public void expandForNewInsertedEdgeOutgoing(Node srcNode, Integer
	// srcNodeId, String srcLabel, Node destNode,
	// Integer destNodeId, String destLabel, boolean srcIsAFocus, boolean
	// destIsAFocus,
	// PrefixTreeNode<IPrefixTreeNodeData> prefixTreeNode, HashSet<Integer>
