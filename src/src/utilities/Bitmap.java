...
public class Bitmap {
	...
	public RoaringBitmap[] storeAllPatternIndexByNodeId(IPrefixTree prefixTree) {
		...
		rBitmapsOfNodes = new RoaringBitmap[(int) Math.max(prefixTree.getLabelAdjacencyIndexer().maxNodeId + 1,
	...
	public void removeOnePatternIndexForAllNodesHavingIt(Integer patternPrefixTreeNodeIndex) {
		for (int i = 0; i < rBitmapsOfNodes.length; i++) {
			rBitmapsOfNodes[i].remove(patternPrefixTreeNodeIndex);
		}
	}
	...
	public void removeNodeIdFromPatternId(Integer destNodeId, Integer patternPrefixTreeNodeIndex) {
		...
		rBitmapsOfNodes[destNodeId].remove(patternPrefixTreeNodeIndex);
